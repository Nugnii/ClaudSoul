...
def _fastembed_version() -> str:
    ...
    except PackageNotFoundError:  # окружение без метаданных пакета
        return "unknown"


# Подпись индекса = модель + ВЕРСИЯ fastembed. Одного имени модели мало: fastembed 0.8
# сменил способ построения вектора внутри той же модели («now uses mean pooling instead of
# CLS embedding»), из-за чего база, собранная прежней версией, и новые запросы оказались в
# разных пространствах. Имя модели при этом не менялось → авто-реиндекс не срабатывал, и
# подбор знаний деградировал молча, сохраняя вид работающего. Версия в подписи делает такое
# обновление наблюдаемым: сменилась библиотека — индекс пересобирается сам.
EMBED_SIGNATURE = f"{EMBED_MODEL}@fastembed-{_fastembed_version()}"

# Каталог кэша модели. Без явного `cache_dir` fastembed кладёт её в
# `tempfile.gettempdir()/fastembed_cache` — на macOS это `/var/folders/…/T/`,
# каталог временных файлов, который система вычищает. 22 августа чистка прошла
# наполовину: `snapshots/` с симлинками осталась, `blobs/` с самими файлами
# опустела, и поиск начал падать с `ONNXRuntimeError NO_SUCHFILE`. Отказ молчит
# до первого вызова, а вызывают семантический поиск не каждую сессию — поломка
# прожила незамеченной, подменяясь grep-ом по именам файлов.
# Поэтому кэш живёт рядом с базой индекса (`~/.claude/knowledge.db`), в
# постоянном каталоге. `FASTEMBED_CACHE_PATH` уважается: сервер запускает
# харнесс, а не шелл, и переменной там может не быть вовсе — умолчание обязано
# быть верным само по себе.
DEFAULT_CACHE_DIR = Path.home() / ".claude" / "fastembed-cache"


def cache_dir() -> Path:
    """Каталог кэша модели. Env читается при вызове, а не при импорте: порядок
    импорта модулей не должен решать, увидим ли мы переменную.

    `~` раскрывается вручную: сервер запускает харнесс из JSON-конфига
    (`mcpServers.claudsoul.env`), а не шелл, и тильду там никто не разворачивает.
    Без этого `~/.claude/fastembed-cache` создавал каталог с именем `~` относительно
    рабочего каталога процесса — то есть ровно ту непредсказуемость, ради ухода от
    которой кэш и переезжал.
    """
    return Path(os.environ.get("FASTEMBED_CACHE_PATH") or DEFAULT_CACHE_DIR).expanduser()
...
def get_model() -> TextEmbedding:
    ...
    if _model is None:
        ...
        _model = TextEmbedding(EMBED_MODEL, cache_dir=str(target))
    return _model
