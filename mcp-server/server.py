...
from indexer import index_all, embed_text, scan_knowledge_dir, scan_domains, KNOWLEDGE_DIR, EMBED_SIGNATURE
...
def _is_reindex_needed(db) -> bool:
    ...
    # Смена модели ИЛИ версии fastembed → все векторы устарели, нужен полный реиндекс
    if get_meta(db, "embed_model") != EMBED_SIGNATURE:
        return True
...
async def _do_reindex(db) -> int:
    ...
    set_meta(db, "embed_model", EMBED_SIGNATURE)
    total = len(entries)

    # Страж: записей в knowledge и векторов должно быть поровну. Без него потеря векторов
    # невидима — реиндекс рапортует «317 файлов», а поиск возвращает пустоту БЕЗ ошибки
    # (так и было, когда векторная таблица пересоздавалась, а vec-строки писались UPDATE'ом
    # по несуществующему rowid). Успех, который не проверяет свой предмет, — не успех.
    vec_count = db.execute("SELECT count(*) FROM vec_knowledge").fetchone()[0]
    if vec_count != total:
        log.error(
            f"Индекс рассинхронизирован: знаний {total}, векторов {vec_count}. "
            "Поиск будет неполным или пустым."
        )
