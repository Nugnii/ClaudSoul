...
def _ensure_vec_metric(db: sqlite3.Connection) -> None:
    """Снести векторную таблицу, если она создана со старой (L2) метрикой.

    `CREATE VIRTUAL TABLE IF NOT EXISTS` молча оставил бы прежнюю схему, и база, собранная
    до перехода на косинус, продолжала бы выдавать мусор — поломка невидимая, потому что
    поиск при этом «работает» и что-то возвращает. Пересоздание пустое: эмбеддинги
    восстанавливаются реиндексацией, которая и так запускается при рассинхроне с диском.
    """
    row = db.execute(
        "SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'vec_knowledge'"
    ).fetchone()
    if row and row[0] and "cosine" not in row[0].lower():
        db.execute("DROP TABLE vec_knowledge")
        # Сбрасываем подпись модели → `_is_reindex_needed` увидит расхождение и наполнит
        # индекс заново. Таблицы meta может ещё не быть (первый запуск) — это не ошибка.
        try:
            db.execute("DELETE FROM meta WHERE key = 'embed_model'")
...
def init_db(db: sqlite3.Connection) -> None:
    ...
    """)
    # Метрика — КОСИНУСНАЯ, и это не деталь вкуса. vec0 по умолчанию меряет L2, а
    # эмбеддинги fastembed не нормализованы (замер: L2-норма 2.56 и 4.52 на двух запросах),
    # поэтому евклидово расстояние определялось длиной текста, а не смыслом: на запрос про
    # SSE/nginx выдавались webhook SendPulse и «угадывание домена», а знание, слово в слово
    # совпадающее с запросом, оказывалось третьим. Отрицательная «релевантность» в выдаче
    # (-282%) была следствием того же: server.py печатает `1 - distance`, формулу для
    # косинуса, а получал L2 в диапазоне 2-4. С cosine distance = 1 - cos эта формула верна.
    _ensure_vec_metric(db)
    db.execute("""
...
def upsert_knowledge(
    ...
    if existing:
        ...
        # DELETE+INSERT, а не UPDATE: если строки в векторной таблице нет (её пересоздали
        # при смене метрики, файл БД потеряли, ручная чистка), UPDATE обновит НОЛЬ строк и
        # вектор пропадёт молча — реиндекс при этом отрапортует «317 файлов», а поиск будет
        # возвращать пустоту без единой ошибки. Так и случилось при переходе на косинус.
...
def graph_data(db: sqlite3.Connection) -> dict:
    ...
    # Relation-файлы не являются узлами (пропущены в первом проходе), но их id
    # попадает в path_to_id — рёбра, ссылающиеся на них, висячие и ломают отрисовку
    # ("node not found: N" в force-graph). Отбрасываем: связь между сущностями уже
    # построена в 2b напрямую.
