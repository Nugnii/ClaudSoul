...
| Расширение | Backend | Файл | Примечания |
|-----------|---------|------|-----------|
| `.md` | `pass_through` | `ingest/readers/pass_through.py` | Читаем как есть |
| `.txt` | `text_wrap` | `ingest/readers/text_wrap.py` | Оборачиваем в code block, heading = имя файла |
| `.pdf` | `pymupdf4llm` | `ingest/readers/pymupdf4llm_reader.py` | Default. Opt-in `--high-quality` → marker |
| любое (fallback) | `markitdown` | `ingest/readers/markitdown_reader.py` | Универсальный разбор, когда профильного бэкенда нет |
