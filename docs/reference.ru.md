...
| `completion-gate` | Stop: пункт, объявленный ☑, сверяется со СВОИМ критерием, а не с отчётом. |
...
| `semantic-prefetch` | UserPromptSubmit: фоновый предкэш семантического поиска по теме реплики. |
