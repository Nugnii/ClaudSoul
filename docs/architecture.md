...
**Схождение доказывает родство, а не корень** (правка 30 августа 2026, ADR-016): у узла с
...
своей починкой; подъём к общему узлу допустим только с непадающей дробью. Прежняя редакция
требовала «поднимайся, пока звено не накроет все проявления» — правило запрещало исход «два
корня разного уровня» и выталкивало вверх ровно в момент расхождения, потому что ниже точки
...
находку О СЕБЕ: с 30 августа 2026 признание собственной ошибки («моя ошибка», «прочитал
неверно») тоже повод, потому что прежде ошибку системы ловил механизм, а ошибку агента
не ловил никто. Восьмой —
...
проверки дрейфа — всё это раньше наблюдалось и оставалось строкой в отчёте скрипта.
...
BACKLOG проекта, BACKLOG ClaudSoul, база знаний), `none`. Успех и отказ добавлены потому,
что без них в знаменатель попадали только неудачи и доля «без исхода» была завышена по
...
**Знаменатель есть и у соседних стражей.** `blocker-tier-check`, `user-correction-guard`,
`rework-detector`, `docs-family-check` писали журналы ПОСЛЕ решения сказать, поэтому доля
«сказал против промолчал» по ним была непосчитаема — тот же корень, что у гейта разбора.
Журнал `state/detection-<SID>.jsonl` фиксирует совпадение признака с пометкой `said` либо
`muted` и ПОДПИСЬЮ события (файл и цикл переделки, хеш реплики, ключ троттла); замер
`detection-share` сводит записи по подписи — событием считается разная подпись, а не вызов
инструмента, иначе доля считалась бы от объёма работы (первый прогон 30 августа 2026 так и
считал: одна реплика на каждом вызове давала «сказал 14%»). У `rework-detector` троттл сужен
до (файл, число циклов): новый цикл переделки — новое событие, страж говорит снова. Отдельно `dead-sensor-check` называет признаки,
...
| `detection-signals-lib.sh` | Pure evaluator: `ds_has_blocker_flag`, `ds_extract_signals`, `ds_evaluate` (все без состояния). Состав матчеров и композиторов здесь НЕ перечисляется — единый источник `knowledge/META.md`, раздел про `detection_signals`: перечень в двух местах разъехался ещё до 28 августа 2026 (в этой строке не хватало `command_uses`, `tool_input_regex`, `heredoc_body_regex`), и обновлять его пришлось бы вручную при каждом новом предикате. |
...
источник). Прежде дубль стоял, и 27 августа 2026 замер показал, во что это обошлось:
пять статусов из пятнадцати разошлись с файлами, мост L2↔L2 отсутствовал вовсе, а
заголовок обещал пятнадцать при шестнадцати существующих. Стража на эту поверхность не
было и не нужно — нечего сторожить, когда нечему расходиться (D97).
