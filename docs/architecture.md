...
показания в архив не переносится. Показание пункта — число, которое считается, а не
