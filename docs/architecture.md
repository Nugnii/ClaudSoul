...
> Версия 1.3 (ревью 2026-05-06) — спецификация когнитивной модели AI-агента. Покрывает реализацию через v1.24.0. Это документ состояния: версии-аннотации в шапке не копятся, хронология — в `CHANGELOG.md`.
...
✅ Contradiction lineage: `modification_history` вместо скалярного `contradicted_count`, `fragile` auto-flag при contradicted_count ≥ 2, блокирует автопромоушен, маркер ⚠️ в knowledge-activator, Section 5c.1 в /retro для ручного разбора fragile знаний.
