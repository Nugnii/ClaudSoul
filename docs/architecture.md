...
> Версия 1.3 (ревью 2026-05-06) — спецификация когнитивной модели AI-агента. Покрывает реализацию через v1.31.2. Это документ состояния: версии-аннотации в шапке не копятся, хронология — в `CHANGELOG.md`.
...
|------|--------|-------|--------|--------|
| 1. Persistence | Как пережить конец сессии? | Контекст работы, PreCompact | SESSION.md, memory, git, auto-scanner, session registry (UUID), chunk boundary snapshots | ✅ Реализован |
| 2. Knowledge | Что мы знаем и насколько уверены? | События, ошибки, успехи | Cases, patterns, principles + demand + tier/scope + lineage + blocker-tier + cross-contour surfacing + async semantic ranker + union filter + H10 surfacing metric | ✅ Реализован |
| 3. Communication | Что происходит между агентом и собеседником? | Диалог, реакции, решения | Intent gaps (literal/pragmatic/strategic), decision patterns, interlocutor model, auto-collection outcomes, distressed state axis как infrastructural brake | ⚡ Частично (retro + classifier + auto-collection outcomes; decision patterns и satisfaction signals всё ещё retrospective) |
...
| 5. Meta-Cognition | Правильно ли мы учимся? | Метрики, аномалии, тренды 7d/30d + intrusiveness history + H9-H12 инструментация | Коррекции процесса и знаний, калибровка gate | ✅ Реализован (metrics-collector + delta trending + intrusiveness trends + H9-H12 + chunk boundary в калибровочном окне) |
| 6. Prediction | Что будет дальше? | Trajectory + Interlocutor Model + 4D axes (confidence × value × cost × state) | Каскадные P_n + downgrade ladder (proactive_action / gentle_suggestion / silent_prep / ignore) | ✅ Реализован (4D gate + active state + state classifier + blocker-tier + auto-collection outcomes) |
| **Мосты** | **Какие навыки рождаются на пересечении?** | **Выходы нескольких слоёв** | **Эмерджентные способности (§10)** | **16 мостов; разбивка по статусам — `bridges/_index.md` §Статистика** |
| 7. Co-Cognition | Как мыслить вместе? | Всё вышеперечисленное | Совместное понимание + disagreement outcome logging | ⚡ Подтверждён (+ каскадные исходы ⚡-событий) |
...
- **Engineering escalation.** Рекурсивная meta-defense против inside-out-blindness самого паттерна. `knowledge-audit-digest.sh` во втором проходе сканирует `pattern-*.md` + `principle-*.md` на `blocker: true` + parseable `escalation_threshold` + `confirmed_count ≥ threshold` — при совпадении пишет секцию `## ⚠️ Engineering escalation needed` в digest, hint `🛠️ Engineering escalation: N blocker-tier pattern(s) crossed escalation_threshold` в `state/audit-hint.txt` с приоритетом выше overdue/trend → session-start startup-signals. Правило-в-тексте полагается на «future-agent прочитает и применит», правило-в-механизме активируется автоматически на семантическую акцию. Если blocker-tier pattern продолжает подтверждаться несмотря на detection_signals — значит они неполны или есть новое измерение; система не должна полагаться на то, что человек заметит. Embedded реализация: `escalation_threshold` + `escalation_hint` в frontmatter, digest surface'ит сам. [principle-knowledge-in-the-world](../knowledge/principle-knowledge-in-the-world.md) (Don Norman, promotion_tier 3, scope universal) формализует уровни embedded-ness fix'а после подтверждённого инцидента: **(1) text rule** (хрупкое — черновик формулировки, до реализации механизма), **(2) activator injection** (правило с якорями, попадает в inject при совпадении контекста), **(3) blocker-tier detection** (detection_signals + хук с адресным инжектом), **(4) отказ через harness** (`permissionDecision: deny` либо `ask` — вызов не проходит без решения). Fix после подтверждённого инцидента никогда не должен оставаться на уровне 1.

  **Уровень 3 — адресный инжект, а не гейт** (надёжность измерена, ADR-012): `blocker-tier-check.sh` по умолчанию заканчивается `additionalContext`. Отказ появляется только у отдельного сигнала знания с полем `enforcement: deny` → `permissionDecision: deny`; отказ адресован агенту, собеседника не трогает (ADR-011). Отказ через harness реализован у 5 хуков (`hooks_deny` в `scripts/count-stats.sh`).
...
surprise_bonus = intensity × max(0, 1 - days_since_created / 180)
  — затухающий множитель: свежий сюрприз активируется
    ...
    Дни здесь — дни ОПЫТА (D234): прожитые сессии реестра /
    темп FSRS_SESSIONS_PER_DAY, календарь — лишь фоллбек
    без реестра (fsrs_experience_days_since)
```
...
> - Базовые веса по типу якоря (situation/trigger/stakes = 3, domain/environment/circumstances/purpose/method = 2; actors веса не имеет — в момент активации действующее лицо всегда агент, различающая сила нулевая)
...
Contradicted знание не удаляется и не сливается в одно поле `contradicted_count`. Ведутся ДВА лога, и разнесены они намеренно: `modification_history` — перекройки самого правила (narrowed / branched / deprecated / reinforced_after_challenge / scope_widened), пишется руками скиллами; `provenance_log` — почему сработало каждое подтверждение или противоречие, пишется механически `knowledge-counter-bump.sh`. Это каскадный аналог скалярного счётчика: без истории «один contradicted» не отличим от «4 независимых contradicted подряд от разных собеседников», а формула fragile, считай она по общему списку, метила бы «хрупким» любое хорошо подтверждённое знание — подтверждения в базе преобладают.
...
«Дни» формулы — дни ОПЫТА, не календаря (D234, case
experience-not-calendar-as-denominator): прошедшее с last_confirmed
считается уникальными сессиями реестра (~/.claude/sessions/registry.jsonl),
делёнными на темп FSRS_SESSIONS_PER_DAY (=30, медиана сессий за активный
день). Полгода тишины = 0 прошедшего — база не гниёт в critical без
опровержений; день с 1000 сессий = ~33 дня опыта. Календарные дни — фоллбек
там, где реестра нет (чистая установка, CI). Реализация:
fsrs_experience_days_since в hooks/fsrs-lib.sh.
```
...
### Статус реализации
✅ Реализован: формат знаний v0.2, 9 якорей, реляционные веса, demand-компоненты, жизненный цикл, /retro, /learn, knowledge-activator hook с weighted scoring и demand-бонусом. Граф доменов: 27 узлов (7 корневых), графовый обход в knowledge-activator (weight: self×30, parent/child×20, overlaps×10, analogous×5).
...
✅ Contradiction lineage: два лога — `modification_history` (перекройки правила, руками) и `provenance_log` (почему подтвердилось/разошлось, механически); скаляр `contradicted_count` остаётся счётчиком, историю несут логи; `fragile` auto-flag при ≥3 перекройках, блокирует автопромоушен, маркер ⚠️ в knowledge-activator, Section 5c.1 в /retro для ручного разбора fragile знаний.
...
✅ MCP fallback в knowledge-activator: когда keyword-top-score < 3 или результатов < 2, хук сперва берёт свежий предкэш от `semantic-prefetch` (кэш жив одну реплику — свежесть по маркеру `.prompt`, не по TTL); протухший или отсутствующий кэш — синхронный вызов `mcp-server/cli_search.py` (обёртка над `storage.search` + `indexer.embed_text`), и инжект дополняется семантически релевантными знаниями. Маркер `score=99` в injection-log. Закрывает класс ошибок «low-overlap high-relevance knowledge» (кейс `case-2026-04-16-ignored-own-knowledge-base` — `pattern-inside-out-blindness` пропускался keyword-scoring'ом при confidence 4).
...
✅ Hybrid-предкэш семантики (D233): фоновый prefetch по cwd+topic на реплике собеседника (`hooks/semantic-prefetch.sh`, UserPromptSubmit); fallback читает кэш вместо синхронного вызова, кэш жив одну реплику (маркер `.prompt`, D234).
⬚ Не реализовано: Domain Graph Фаза 2: миграция в БД.
...
### Контур разбора причин (модуль root-cause)

Определение корня проблемы — общесистемный контур, а не приём отдельного скилла. Его
единственный источник — `hooks/root-cause-lib.sh`: там живут доктрина (текст порядка),
метка хода, счёт цепочки, перечень поводов и журнал исходов. Потребители — гейт разбора
(`five-whys-gate`, единственный принуждающий), сверщик исхода (`declared-problem-recorded`,
Stop), тексты `error-tracker` и `rework-detector`, поводы `revert-signal`; через журнал
знаменателя `state/detection-<SID>.jsonl` (`rc_note_detection`) — `blocker-tier-check`,
`docs-family-check`, `user-correction-guard`, `rework-detector`; человеческие носители доктрины
(`rules/CLAUDE.md` Step 2/4, `/retro` Step 3, `/adversary` Step 4) держит drift-страж.

**Порядок:** проявления с адресами → цепочки «почему» от 2-5 максимально разных → сверка
схождения → **братья корня** (вбок: то же устройство там, где симптома ещё не видно) →
зависимости корня (вниз). **Последствия починки** — та же карта зависимостей, прогнанная
после правки: починка корня ломает то, что на прежнее поведение опиралось, включая костыли
поверх дефекта.

**Спуск идёт двумя линиями:** вдоль исполнения («почему сломалось здесь») и вдоль данных
(«откуда пришло то, из чего считали»). У места, где ошибка стала видна, перечисляются все
входы; ветки с верным входом отсекаются, с неверным — спуск продолжается туда. Узел,
исправно посчитавший неверный вход, корнем не является, и признак наблюдаем: подай верный
вход — симптом исчезнет без правки узла. Источников на этой линии бывает несколько, и все
настоящие; дно — последнее звено внутри того, что меняется своим коммитом.

**Остановка — по признаку звена, не по числу звеньев,** и признаков четыре: убери звено, и
симптом не мог бы возникнуть ни одним путём (даёт необходимость, не корень, и легчает при
подъёме); **дробь специфичности** — N мест со звеном, M из них с симптомом, предъявляется
числом, и шаг вверх, роняющий дробь, есть шаг за корень; звено предсказывает третье место;
на уровне корня формулируется признак, у которого продолжение вызова имеет одно
последствие, и назван адрес отказа. Дробь и признак-с-одним-последствием — единственные
два теста, ТЯЖЕЛЕЮЩИЕ при подъёме, и потому единственные встречные дрейфу вверх.

**Схождение доказывает родство, а не корень** (ADR-016): у узла с
несколькими входами — сумматора, журнала, отчёта — сходится что угодно, потому что там
сходятся входы. Расхождение цепочек означает по умолчанию **несколько корней**, каждый со
своей починкой; подъём к общему узлу допустим только с непадающей дробью. Требование
«поднимайся, пока звено не накроет все проявления» здесь непригодно: оно запрещает исход
«два корня разного уровня» и выталкивает вверх ровно в момент расхождения — ниже точки
схождения ни одно звено не накрывает всё по построению.

**Поводов восемь**, и все читаются у чужих продюсеров — своего счётчика контур не заводит.
Семь родятся внутри сессии: переделка файла, полоса провалов, поправка о поведении, вопрос
собеседника, повтор подтверждения знания за сессию и за сутки, находка по ходу — включая
находку О СЕБЕ: признание собственной ошибки («моя ошибка», «прочитал неверно») — тоже
повод, потому что ошибку системы ловят механизмы, а ошибку агента иначе не ловит никто.
Восьмой —
**внешнее событие**: мир подаёт поводы, которых в сессии не видно, и зрелые практики
запускают разбор именно на них (у Google SRE откат релиза обязывает к разбору сам по себе,
в медицине девять классов событий репортятся независимо от исхода). Красный прогон СВОЕГО
коммита, откат коммита и повторный hotfix, воскресшее закрытие пункта долга, сбой самой
проверки дрейфа — события, которые видят только скрипты, а не диалог.
Носитель у них один — `state/pending-events.jsonl` без идентификатора сессии, потому что
два продюсера суть скрипты по расписанию; событие гаснет, когда исход хода записан.
Прогоны в фикстурах поводом не становятся: `rc_env_is_real` требует непеределанного
окружения — правило, а не перечень известных тестов.

У КАЖДОГО повода пишется исход в `state/outcome-<SID>.jsonl`, определённый наблюдаемо.
Значений четыре: `chain-spoken` (цепочка произнесена — единственный исход, означающий УСПЕХ
стража), `deferred` (правка отложена отказом), `recorded` (носитель изменился: локальный
BACKLOG проекта, BACKLOG ClaudSoul, база знаний), `none`. Успех и отказ стоят в знаменателе
наравне с прочими: знаменатель из одних неудач завышал бы долю «без исхода» по
построению — а на ней стоит правило подъёма уровня. Это и делает измеримым различие «страж
сработал» и «страж помог»: замер `signal-outcomes` считает по каждому поводу долю без
исхода и долю ложных и называет, чего число требует — поднять уровень принуждения или
сузить признак. Окно замера — в сессиях, не в днях.

**Разбор оставляет след, по которому его можно проверить** (ADR-017). Рядом с исходом в
записи стоит поле `trace` — формы доктрины, найденные в тексте хода: слово «почему», разные
адреса `файл:строка`, дроби «N из M» с N < M, слово «корень», заголовки шаблона, спуск «←»,
названные входы. «Цепочка произнесена» определяется одной формулой (`rc_chain_spoken`):
«почему» ≥ 3, либо два заголовка шаблона, либо три «←», либо реестр мест с дробью и корнем;
формы выбраны реплеем по корпусу расшифровок, а не назначены, и тот же замер печатает долю
поводов со следом против базы корпуса. Предел назван: это формы, а не мысль, и цена ложного
«след есть» — правка, прошедшая без цепочки; снимется, когда след будет записываться не из
речи.

**Знаменатель есть и у соседних стражей.** У `blocker-tier-check`, `user-correction-guard`,
`rework-detector`, `docs-family-check` журнал `state/detection-<SID>.jsonl` фиксирует
совпадение признака В МОМЕНТ совпадения — с пометкой `said` либо `muted` и ПОДПИСЬЮ события
(файл и цикл переделки, хеш реплики, ключ троттла); запись после решения сказать делала бы
долю «сказал против промолчал» непосчитаемой по построению. Замер `detection-share` сводит
записи по подписи — событием считается разная подпись, а не вызов инструмента, иначе доля
считалась бы от объёма работы: одна реплика, совпавшая на каждом вызове, растворяла бы её.
У `rework-detector` подпись троттла — (файл, число циклов): новый цикл переделки — новое
событие, страж говорит снова. Отдельно `dead-sensor-check` называет признаки,
которые за окно сессий не сработали ни разу: молчание сенсора неотличимо от исправности,
и список молчащих берётся из знаний, а не из журналов — в журналах их нет по построению.

**Связь с долгом.** Исход из закрытого списка (починка · пункт долга · знание · вердикт
«показалось») замыкает цикл: пункт несёт измеримый результат и команду проверки, правка
файла из открытого пункта поднимает его до правки (`backlog-touch-check`), закрытие без
показания либо с красной «Проверкой» в архив не переносится, а «Проверка» ☑-пунктов
прогоняется и на границе хода (`completion-gate`, Stop) — красная называется ещё в
сессии закрытия. Показание пункта — число, которое считается, а не
помнится: пункт объявляет команду, строка показания пересчитывается на каждой границе хода
(Stop, после архиватора) и при правке файла, а при чтении агентом сверяется с миром, и
устаревшее чтение считается замером `stale-reads`. Механические записи в BACKLOG.md —
пересчёт и уборка — сохраняют mtime файла: сверщик исхода хода судит по mtime, и иначе
журнал исходов подделывался бы самим контуром.
...
| H10 | Cross-reference между контурами порождает знания | `mcp-server/ingest/discovery.py::detect_cross_contour_mentions` | `cross-contour-discoveries.jsonl` — счётчик упоминаний entity в case/pattern/principle |
...
**Силент наружу по дизайну.** По умолчанию вывод через `hookSpecificOutput.additionalContext` — невидимый для пользователя, видимый для агента. Не используется `systemMessage` (транскрипт-баннер) и не используется `permissionDecision: "ask"` (останавливает собеседника). Принуждение, где оно есть, — `permissionDecision: "deny"` у сигнала с `enforcement: deny`: отказ адресован агенту, собеседника не трогает (ADR-011). Причина: *«Озвучивать свою рефлексию и не обязательно — это направлено на принятие правильных решений, а не на раздувание и шум»*. Сам шум — отдельный failure mode; баннер «я чуть не ошибся» на каждый Edit обесценивает внимание.
...
| `detection-signals-lib.sh` | Pure evaluator: `ds_has_blocker_flag`, `ds_extract_signals`, `ds_evaluate` (все без состояния). Состав матчеров и композиторов здесь НЕ перечисляется — единый источник `knowledge/META.md`, раздел про `detection_signals`: дублированный перечень расходится молча, и обновлять его пришлось бы вручную при каждом новом предикате. |
...
**Когда помечать как blocker.** Явное решение, не автоматически по `confirmed_count`. Критерии: (1) подтверждённый knowledge-action gap (паттерн в базе, но retrieval не вытаскивает в нужный момент), (2) ситуация распознаётся по измеримым сигналам (не требует интерпретации), (3) `confirmed_count ≥ 5` и `outcome: error`. Пример действующего блокера — `pattern-inside-out-blindness.md` (сигналы и счётчики — во frontmatter знания).

**Граница ответственности.** Два режима, и режим стоит у сигнала, а не у знания (ADR-011). По умолчанию хук **обогащает контекст агента** маркером `🛑 Blocker: <signal>` — уровень 3, адресный инжект (ADR-012); видимая реакция — выбор агента (пересмотреть подход, предупредить пользователя явно, продолжить с поправкой). Сигнал с `enforcement: deny` отвечает `permissionDecision: deny` — уровень 4: вызов не проходит, причина отказа несёт `blocker_reminder` с заменой; среди совпавших сигналов побеждает сильнейший. Это L6-консистентно: где отказа нет, gate-решение остаётся у агента, хук даёт сигнал с высокой уверенностью, где раньше сигнал терялся в retrieval-шуме.

**Cross-hook recall gate (второй режим срабатывания — для multi-dimensional паттернов).** Реализованный ответ на эскалацию, о которой `knowledge-audit-digest.sh` лишь *заявляет*. `pattern-inside-out-blindness` пересёк `escalation_threshold` (заявка на эскалацию закрыта реализацией `fix-level-check.sh`; нынешний `confirmed_count` — во frontmatter знания), и его `modification_history` — прямое доказательство, что `detection_signals` не сходятся: каждое подтверждение — *новое измерение* (длинные docs, zsh, pipefail, memory-gate, собственный output, cross-directory, покрытие, конфиг…). Узкий сигнал на `*.config.*` закрыл бы ровно то проявление и не поймал бы следующее. Домен-независимый инвариант всех проявлений — «создаю/меняю, не проверив внешний контекст»; по содержанию он недетектируем, но **сработавший защитный guard этой сессии** — домен-независимая улика, что система уже задетектила импровизацию. Второй режим: паттерн с opt-in флагом `cross_hook_recall_gate: true` при `Write` после любого allowlisted guard (`correction-fired` / `bulk-copy-fired` / `internal-doc-leak-fired` / `playwright-cli-guard-fired`; переопределяемо через `GUARD_FIRED_MARKERS`) получает инжект своего `cross_hook_recall_reminder`. Throttle per `(pattern, guard)`. Логика — в `blocker-tier-check.sh`, не в `detection-signals-lib.sh` (у неё контракт «без состояния»). **Честный предел:** детект ≠ комплаенс — в `case-2026-06-16-existing-config-blindness` сработавший `playwright-cli-guard` был обойдён сознательно; reminder поднимает вероятность, не заставляет. Жёсткий `permissionDecision: ask` отклонён ради `feedback_silent_correct_decisions` (шум — сам по себе failure mode); gate намеренно редкий (только `Write`) — при высокой доле оверрайдов широкий gate стал бы фоном.
...
- **Blocker-tier knowledge** — silent relational pre-action check для подтверждённых knowledge-action gap паттернов: `detection-signals-lib.sh` + `blocker-tier-check.sh`, по умолчанию silent inject, `deny` у сигнала с `enforcement` (ADR-011), throttle per `(pattern, file)` (детали выше в этой секции).
...
- **Hypothesis instrumentation H9-H12** — см. §7 L5. `contradiction.gap_type` в `integrate._merge_scalar_attrs` (H9), `detect_cross_contour_mentions` в `mcp-server/ingest/discovery.py` + `cross-contour-discoveries.jsonl` (H10), `cascading.backward_count` в history digest через `reformulation-tracker` BACKWARD-счётчик (H11), `cost_peaks.injection_bytes_max` в history digest (H12).
...
Полный перечень со статусами — **`bridges/_index.md`**; источник истины по каждому
статусу — frontmatter самого файла `bridges/L*.md`, и его сверяет тест
`test_bridge_status_consistency`.

Здесь таблица не дублируется намеренно (ADR-001: один факт — один авторитетный
источник): дубль расходится молча — статусы, перечень мостов и их число. Стража на
эту поверхность не нужно — нечего сторожить, когда нечему расходиться.
...
| Demand-мышление | "Вы курите?" | Пункт 0 (правило + инжект активатора, уровень 2) | ⚡ |
...
| **D** | **Спонтанная аналогия (insight-момент)** | «О, это как тогда с X» — проактивный cross-domain surfacing | **Полный цикл детект → ранжирование → инжект → метрика.** Upstream: `knowledge-audit-digest.sh` (launchd вс 03:15) python-heredoc проход `detect_cross_contour_mentions` над live entity/knowledge → дописывает `cross-contour-discoveries.jsonl` (детекция не прикована к ручному `/ingest`). Ranker: `mcp-server/cli_cross_contour_rank.py` batch-embeds уникальные KF/EF (через `indexer.embed_texts` → текущая `EMBED_MODEL`), cosine similarity → `cross-contour-ranked.jsonl`. Запускается тем же weekly digest — async pre-computation, PreToolUse не платит embedding latency. Consumer (`knowledge-activator.sh`) union filter: `(KF в инжект-наборе) OR (similarity ≥ CC_SIMILARITY_THRESHOLD, default 0.6)`. Session-scope dedup: `state/cross-contour-surfaced-${SID}.txt`. Мост `bridges/L2-L2-cross-contour.md` формализует cross-contour как межслойное явление. | ✅ расширенный. Остаются extension оси — L2↔L2 as self-analogy, multi-hop — и калибровка threshold после naked baseline |
...
В `principle-knowledge-in-the-world.md`: timing of surfacing — вектор внутри уровня 2, не свойство уровня. Это не отдельный уровень embedded-ness; лестница остаётся 1/2/3/4 (text rule / activator injection / blocker-tier hook / отказ через harness, ADR-012). Внутри уровня 2 явно две оси: канал inject (UserPromptSubmit / PreToolUse) и timing (turn-grained / tool-call-grained).
...
### Разрыв «repository ≠ installed» — ортогональная ось proof-of-active
...
Лестница embedded-ness из `principle-knowledge-in-the-world` (1-4: text rule / activator injection / blocker-tier hook / отказ через harness) описывает, как fix **встроен** в систему. Но встроенность в репо ≠ активность на машине — это ортогональная ось, а не ступень лестницы. **Proof-of-active**:
...
- **Schema:** `ITR_SCHEMA_VERSION=4` несёт `state.distribution.distressed`; state-файлы прежних версий мигрируют идемпотентно через `_itr_migrate_state`
...
- **Задача блока:** `silence_debt` (подавленные высокоценные интервенции) копится в `intrusiveness-<SID>.json`, и без переноса он не пересекает границу сессии — долг знал бы только state-файл прошлой сессии, до которого никто не добирается; cost-of-silence был бы видим только внутри сессии.
- **Трёхчастная реализация (infrastructural, не text rule):**
  1. **Durable digest** — `itr_append_history` пишет блок `debt: { surfaced, pending, pending_topics[:5] }` в `intrusiveness-history.jsonl` на каждом chunk boundary (Stop + PreCompact). Это durable carrier между сессиями.
...
Виджеты в едином космическом стиле:
