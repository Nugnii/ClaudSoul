...
provenance_log:
  ...
  - date: 2026-08-26
    kind: reinforced
    reason: "дважды удержало от ложного диагноза: подсчёт 245 pending оказался моей ошибкой чтения, а не дефектом счётчика; странный результат уборки — моим запуском в zsh, а не дефектом правки"
    trigger_case: case-2026-08-26-keep-list-is-a-copy-not-a-derivation.md
  - date: 2026-08-26
    kind: reinforced
    reason: "сигнал BACKWARD о своей ошибке не принят как дефект хука сразу: сперва проверены свои данные (реплика владельца коррекции не несёт), затем найден механизм в reformulation-tracker.sh:76 и intrusiveness-classify-lib.sh:92 — вывод про инструмент сделан после проверки, а не вместо неё; результат D87"
  - date: 2026-08-30
    kind: reinforced
    reason: "30.08.2026: rework-detector сказал «правится 3-й раз», я диагностировал дефект инструмента (ключ по basename), не проверив данные — счётчик считает по полному пути (:169-180), журнал подтвердил 4 правки того же файла"
    session: ""
---
