...
`fragile: true` при 3+ записях `modification_history` (там только перекройки правила; провенанс подтверждений живёт в `provenance_log` и на флаг не влияет) — блокирует промоушен в principle и дополнительно увеличивает `source_factor × 1.2` при следующих противоречиях.
