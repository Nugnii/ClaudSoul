...
if [ "$FAILS" -ge "$THRESHOLD" ]; then
    ...
    if [ "$LAST_FIRED" != "$FAILS" ]; then
        ...
        # Мост данных для жнеца опровержений (D229): у файла-флага нет времени, и провал
        # не привязывался к моменту — джойн «инжект × провал» был невозможен. Журнал
        # дозаписывается только при смене длины серии (тот же гейт, что у сообщения);
        # файл-флаг выше не меняется — его читатели остаются как были.
        printf '{"ts":"%s","attempts":%s}\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$FAILS" \
            >> "$STATE_DIR/error-streak-log-${SESSION_ID}.jsonl" 2>/dev/null || true
