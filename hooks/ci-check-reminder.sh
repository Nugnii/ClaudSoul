#!/usr/bin/env bash
# ci-check-reminder.sh — PostToolUse[Bash]: после `git push` напоминает проверить прогон СВОЕГО коммита, а не «самый свежий».
...
grep -qE '(^|[[:space:]]|;|&&|\|\|)git[[:space:]]+push([[:space:]]|$)' <<< "$EXEC" || exit 0
...
# Два канала: `systemMessage` владельцу, `additionalContext` в контекст хода. Один
# только `systemMessage` до хода не доходит — записи `hook_system_message` в поток
# сообщений не попадают (замер по истории расшифровок 2026-08-21).
jq -n '
  "🚦 Пуш ушёл. Прогон проверяй для СВОЕГО коммита, не «самый свежий»: bash scripts/ci-status.sh — сразу после push самым свежим числится ПРЕДЫДУЩИЙ прогон, и `gh run list -L 1` дважды выдал ложное «зелено»." as $msg
  | {
      systemMessage: $msg,
