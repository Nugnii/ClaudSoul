#!/usr/bin/env bash
# code-review-reminder.sh — PreToolUse[Bash] на `git commit`: крупный кодовый дифф идёт в коммит без адверсариального прогона — поручает СКАЗАТЬ собеседнику про `/adversary`.
# en: PreToolUse[Bash] on `git commit`: a large code diff heading into a commit without an adversarial run — instructs to tell the interlocutor about `/adversary`.
#
...
#    поиск подтверждения вместо поиска ошибки. Теперь зовёт `/adversary`:
...
# расшифровки (скилл `adversary`, субагент с его промптом либо воркфлоу, гоняющий
...
[ -z "$NUMSTAT" ] && exit 0
# КОНТРПРИМЕР: расширения вне перечня — `.vue`, `.svelte`, `.zig`, `.lua`, `.tf` — кодом
# не считаются и напоминания о ревью не получат. Перечень назван перечнем: он растёт по
# мере встречи новых языков в проекте, а не выводится из правила «это исходный код».
...
# Skill{skill:"adversary"}, Agent с промптом противника либо Workflow, чей скрипт
# этот промпт содержит.
#
# ИМЯ СКИЛЛА И МАРКЕР ПРОМПТА — РАЗНЫЕ ФАКТЫ, и до переименования 28.08 они случайно
# совпадали. Скилл зовётся `adversary`, а роль внутри его промпта осталась русской
# («Ты противник»): вызов Skill опознаётся по первому, вызов Agent/Workflow — по
# второму. Слить их обратно в одну переменную значит потерять половину детекта.
#
...
SKILL_NAME="${ADVERSARY_SKILL_NAME:-adversary}"
PROMPT_MARKER="${ADVERSARY_PROMPT_MARKER:-Ты противник}"
...
if [ -n "$TRANSCRIPT" ] && [ -f "$TRANSCRIPT" ] &&
   { grep -qF "$SKILL_NAME" "$TRANSCRIPT" 2>/dev/null ||
     grep -qF "$PROMPT_MARKER" "$TRANSCRIPT" 2>/dev/null ||
     grep -qF '"scriptPath"' "$TRANSCRIPT" 2>/dev/null; }; then
  RAN=$(jq -rs --arg s "$SKILL_NAME" --arg m "$PROMPT_MARKER" '
    [ .[]
      | (.message.content? // [])
      | if type == "array" then .[] else empty end
      | select(type == "object" and .type == "tool_use")
      | if (.name == "Skill" and ((.input.skill // "") == $s)) then 1
        elif (.name == "Agent" or .name == "Task") and (((.input.prompt // "") | contains($m))) then 1
        elif (.name == "Workflow") and (((.input.script // "") | contains($m))) then 1
        else empty end
    ] | length
  ...
  if [ "$RAN" -eq 0 ]; then
    ...
    while IFS= read -r WF; do
      [ -n "$WF" ] && [ -f "$WF" ] || continue
      if grep -qF "$PROMPT_MARKER" "$WF" 2>/dev/null; then RAN=1; break; fi
    done <<< "$WF_PATHS"
  fi
