...
# восьмеричными escape, и ни один фильтр путей ниже их не узнаёт. У проекта два таких
# скилла из двадцати трёх (`противник`, `грилинг`) — источники автотаблиц, которые
# путь A2 обязан видеть. Тот же дефект чинили 22-23.08 в трёх соседних стражах; сюда
# правка не дошла, потому что фильтр путей здесь появился тем же утром.
STAGED=$(
