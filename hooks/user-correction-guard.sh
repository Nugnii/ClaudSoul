#!/usr/bin/env bash
# user-correction-guard.sh — PreToolUse: собеседник только что поправил — следующий вызов инструмента останавливается вопросом, пока поправка не переформулирована.
# en: PreToolUse: pause when the user has just corrected me, before I take another tool action.
#
