...
# ============================================================================
# D45 — ротация лога инжектов; знаменатель ротации — сессии (D234)
# ============================================================================
# T12: писатель содержит ротацию, и она по СЕССИЯМ, а не по числу строк и не по
# календарю. Обрезка «оставить последние N строк» разорвала бы пару инжект↔поправка в
# середине сессии; обрезка по возрасту морила сессионные окна приборов после паузы.
KA="$HOOKS_DIR/knowledge-activator.sh"
grep -q 'INJECTION_LOG_KEEP_SESSIONS' "$KA" && ok || bad "T12a" "ротации лога инжектов по сессиям нет"
...
# T14: ротация переносит строки старых сессий (и строки без session_id — журнал до
# ts-моста) целиком, сохраняет сумму и не трогает битые строки. Зовётся настоящий
# писатель — hooks/lib/injection-log-rotate.py (D236), копий блока больше нет.
if command -v python3 >/dev/null 2>&1; then
    LOG="$TMP/injection-log.jsonl"; ARCH="$TMP/injection-log-archive.jsonl"
    python3 - "$LOG" <<'PY'
import json, pathlib, sys
p = pathlib.Path(sys.argv[1])
rows = [json.dumps({"date": "2026-04-01", "file": "legacy.md"}) + "\n" for _ in range(5)]
for s in range(12):                      # 12 сессий по 2 строки, старшие — первыми
    for j in range(2):
        rows.append(json.dumps({"date": "2026-08-01", "file": f"k{s}-{j}.md", "session_id": f"s{s}"}) + "\n")
rows.append("это битая строка, не json\n")
p.write_text("".join(rows))
PY
    BEFORE=$(grep -c '' "$LOG")
    INJECTION_LOG_MIN_BYTES=1 python3 "$HOOKS_DIR/lib/injection-log-rotate.py" "$LOG" "$ARCH" 10 2>/dev/null
    ...
    # keep 10 сессий из 12 → в архив: 2 старшие сессии × 2 строки + 5 строк без session_id
    [ "$ARCHN" -eq 9 ] && ok || bad "T14b" "в архив ушло $ARCHN строк вместо 9"
