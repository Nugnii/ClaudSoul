...
# --- T4: постоянный механизм — второе принятие в той же сессии тоже fire ---
F=$(make_transcript "Беру твой вариант, так проще.")
OUT=$(run_with "sid1" "$F")
assert_contains "$OUT" "принят вариант" "T4: повторное принятие срабатывает"
