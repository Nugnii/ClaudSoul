...
printf '{"session_id":"test-inj-mcp","tool_name":"Bash","cwd":"/tmp/proj","tool_input":{"command":"frobnicate the wibblefitz quux"}}' | \
    env HOME="$TMP/home" CLAUDE_CODE_SESSION_ID="test-inj-mcp" STATE_DIR="$STATE" \
        CLAUDSOUL_ROOT="$STUB" bash "$ACTIVATOR" >/dev/null 2>"$STATE/act-err"
_rc=$?; [ "$_rc" -eq 0 ] || { FAIL=$((FAIL + 1)); echo "FAIL [W3rc]: активатор умер rc=$_rc: $(tail -c 200 "$STATE/act-err" 2>/dev/null)"; }
if [ -s "$LOG" ]; then
...
run_collector() {
    env HOME="$TMP/rhome" LESSONS_DIR="$TMP/rhome/.claude/global-lessons" \
        STATE_DIR="$TMP/rhome/.claude/hooks/state" bash "$COLLECTOR" \
        >/dev/null 2>"$TMP/coll-err" || echo "$?" >> "$TMP/coll-rc"
    cat "$TMP/rhome/.claude/hooks/state/metrics.md"
}
# run_collector зовётся через $(…) — счётчики внутри не выживают; смерть коллектора
# копится флаг-файлом и предъявляется перед итогом (D220).
...
if [ -f "$TMP/coll-rc" ]; then
    FAIL=$((FAIL + 1))
    echo "FAIL [run_collector: коллектор падал, rc: $(tr '\n' ' ' < "$TMP/coll-rc"), stderr: $(tail -c 200 "$TMP/coll-err" 2>/dev/null)]"
fi
