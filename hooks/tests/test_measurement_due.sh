...
OUT=$(run check)
printf '%s' "$OUT" | grep -q 'просрочен: stale'  && ok || bad "T1a" "просроченный не назван"
printf '%s' "$OUT" | grep -q 'просрочен: broken' && ok || bad "T1b" "сломанный не назван"
printf '%s' "$OUT" | grep -q 'просрочен: fresh'  && bad "T1c" "свежий назван просроченным" || ok
printf '%s' "$OUT" | grep -q 'просрочен: evented' && bad "T1d" "событийный попал в просрочку" || ok
printf '%s' "$OUT" | grep -q 'по событию: 1'     && ok || bad "T1e" "событийные не посчитаны"
...
printf '%s' "$OUT" | grep -q 'НЕ ВЫПОЛНЕН' && ok || bad "T3b" "отказ (код 2) не отличён от выполнения"
printf '%s' "$OUT" | grep -q 'ЕСТЬ НАХОДКИ' && ok || bad "T3c" "находка (код 1) не отличена от отсутствия находок"
...
printf '%s' "$(run check)" | grep -q 'просрочен: fresh' && ok \
