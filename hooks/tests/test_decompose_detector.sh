...
# --- Router: ветка решений → /грилинг (v1.x) ---
...
# --- T21: расплывчатый/решения → /грилинг ---
OUT=$(run_with "sid21" "$VAGUE")
assert_contains "$OUT" "/грилинг" "T21: открытые решения → грилинг"
...
if grep -Fq "Multi-step detected" <<< "$OUT"; then
    FAIL=$((FAIL + 1)); echo "FAIL [T22]: грилинг-вывод содержит и decompose-подсказку (двойной инжект)"
else PASS=$((PASS + 1)); fi
...
if grep -Fq "/грилинг" <<< "$OUT"; then
    FAIL=$((FAIL + 1)); echo "FAIL [T23b]: decompose-вывод содержит и грилинг (двойной инжект)"
else PASS=$((PASS + 1)); fi
...
assert_contains "$OUT" "/грилинг" "T25: порог решений 1 — одного сигнала достаточно"
