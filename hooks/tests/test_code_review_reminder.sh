...
assert_contains "$OUT" "/adversary"        "T6b: зовёт противника, а не самокритику"

# --- детект состоявшегося прогона ---

TR_SKILL="$TMP/tr-skill.jsonl"
jq -cn '{message:{content:[{type:"tool_use",name:"Skill",input:{skill:"adversary"}}]}}' > "$TR_SKILL"
...
jq -cn '{message:{content:[{type:"text",text:"обсуждаем скилл adversary и как он устроен"},{type:"tool_use",name:"Bash",input:{command:"echo adversary"}}]}}' > "$TR_TALK"
...
jq -cn '{message:{content:[{type:"tool_use",name:"Workflow",input:{description:"обсуждаем скилл adversary",script:"agent(\"Изучи конвенции.\")"}}]}}' > "$TR_WFD"
