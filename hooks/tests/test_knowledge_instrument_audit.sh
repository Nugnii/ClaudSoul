...
if printf '%s' "$REP" | grep -A100 'Очередь на производство' | grep -B100 'Оценены' | grep -qF 'pattern-assessed'; then
...
if printf '%s' "$(cat "$OUT_MD")" | grep -A100 'Очередь на производство' | grep -B100 'Чего этот замер' | grep -qF 'pattern-assessed'; then
    ok
