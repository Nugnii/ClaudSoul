...
PY
LESSONS_DIR="$L" STATE_DIR="$S" KIA_OUTPUT="$OUT_MD" bash "$SCRIPT" >/dev/null 2>"$S/err"
_rc=$?
# rc=1 законен («отработал и нашёл»); смерть аудита раньше опустошала $OUT_MD молча (D220).
[ "$_rc" -le 1 ] || bad "T13rc" "аудит умер: rc=$_rc, stderr: $(tail -c 200 "$S/err" 2>/dev/null)"
...
LESSONS_DIR="$L" STATE_DIR="$S" KIA_OUTPUT="$OUT_MD" bash "$SCRIPT" >/dev/null 2>"$S/err"
_rc=$?
[ "$_rc" -le 1 ] || bad "T14rc" "аудит умер: rc=$_rc, stderr: $(tail -c 200 "$S/err" 2>/dev/null)"
