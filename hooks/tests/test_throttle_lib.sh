...
assert_eq "$BASH_SURV" "blocker correction rework trust-guard" "T13a: под bash защищённые семейства выжили"
if command -v zsh >/dev/null 2>&1; then
    ZSH_SURV=$(sweep_survivors zsh | tr -s ' ' | sed 's/ $//')
    assert_eq "$ZSH_SURV" "blocker correction rework trust-guard" "T13b: под zsh результат тот же"
