#!/usr/bin/env bash
# test_ablation_pair_cycle.sh — чекер вне песочниц + жизненный цикл тройки:
# hash-реестр, классификация исходов трёх плеч, аннулирование целиком, повтор,
# максимум 2, отказ завершать тройку с недостающим плечом.
...
# --- T3: исходы трёх плеч: full success, core success, vanilla objective_failure ---
fake_arm_run "$ID" full; touch "$ABLATION_DIR/runs/$ID/full/work/done.txt"
fake_arm_run "$ID" core; touch "$ABLATION_DIR/runs/$ID/core/work/done.txt"
fake_arm_run "$ID" vanilla
OUT=$(bash "$AB/checker.sh" run "$ID" full)
assert_contains "$OUT" "success" "T3: full=success"
OUT=$(bash "$AB/checker.sh" run "$ID" core)
assert_contains "$OUT" "success" "T3b1: core=success"
OUT=$(bash "$AB/checker.sh" run "$ID" vanilla)
assert_contains "$OUT" "objective_failure" "T3b: vanilla=objective_failure"
OUT=$(bash "$AB/checker.sh" run "$ID" ядро 2>&1); RC=$?
[ "$RC" -ne 0 ] && ok || bad "T3d" "чекер принял несуществующее плечо"
OUT=$(bash "$AB/checker.sh" run "$ID" full 2>&1); RC=$?
...
assert_contains "$LINE" '"core":1' "T5c1: core=1"
...
fake_arm_run "$ID2" core; touch "$ABLATION_DIR/runs/$ID2/core/work/done.txt"
...
# Недостающее плечо не даёт завершить тройку: без гварда Δ_structure молча
# считался бы по неполному набору (§11).
bash "$AB/checker.sh" run "$ID2" vanilla >/dev/null
OUT=$(bash "$AB/pair.sh" complete "$ID2" 2>&1); RC=$?
[ "$RC" -ne 0 ] && ok || bad "T6d1" "тройка без core завершилась"
assert_contains "$OUT" "все три плеча" "T6d2: причина названа"
bash "$AB/checker.sh" run "$ID2" core >/dev/null
