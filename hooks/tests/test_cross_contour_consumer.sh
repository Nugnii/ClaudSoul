...
run_consumer() {
    ...
    while IFS='|' read -r kf ef matched; do
        ...
        [ "$count" -ge 3 ] && break
        key="${kf}|${ef}"
        if [ -n "$surfaced_existing" ] && grep -Fxq "$key" <<< "$surfaced_existing"; then
            continue
        fi
        keep=false
        if [ -n "$injected_kfs" ] && grep -Fxq "$kf" <<< "$injected_kfs"; then
            keep=true
        elif [ -n "$high_sim_pairs" ] && grep -Fxq "$key" <<< "$high_sim_pairs"; then
            keep=true
        fi
        if [ "$keep" = "true" ]; then
