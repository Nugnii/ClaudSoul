...
OUT=$(run_hook internal-doc-leak-guard \
    "$(jq -cn '{session_id:"s3", tool_name:"MultiEdit", cwd:".",
                tool_input:{file_path:"/x/Lawyer_Track/memo.md", edits:[{old_string:"a", new_string:"текст"},{old_string:"b", new_string:"Для адвоката: черновик"}]}}')")
assert_contains "$OUT" "Internal-doc leak guard" "internal-doc: MultiEdit в той же группе — правки сверяются"
