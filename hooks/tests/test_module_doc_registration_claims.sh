...
#
# Результат: модульный док не называет событие, на котором хук не зарегистрирован, и не
#            называет соседом по группе хук, стоящий в другой группе (D218).
...
# Соседи по группе (D218). «В группе с `a` и `b`» — утверждение о том же дереве: ложное
# «в группе с blocker-tier-check» (rules-write-bypass.md) жило с рождения при зелёном страже,
# потому что сверялись только событие и matcher. Адресат — хук, одноимённый документу.
groups_of = {}
for event, groups in cfg.get("hooks", {}).items():
    for g in groups:
        names = set()
        ...
        for n in names:
            groups_of.setdefault(n, []).append(names)
NEIGH = re.compile(r"в (?:одной |той же )?группе с ((?:`[A-Za-z0-9._-]+`(?:\s*(?:,|и|, и)\s*)?)+)")
for d in docs:
    text = d.read_text(errors="replace")
    globals()["_doc_stem"] = d.stem
    own = doc_hook(text)
    for m in NEIGH.finditer(text):
        if not own:
            continue          # нет одноимённого хука — адресата нет, утверждением не считается
        for n in re.findall(r"`([A-Za-z0-9._-]+)`", m.group(1)):
            n = n if n.endswith(".sh") else n + ".sh"
            checked += 1
            if not any(n in grp for grp in groups_of.get(own, [])):
                mine = sorted({x for grp in groups_of.get(own, []) for x in grp} - {own})
                bad.append((d.name, own, f"сосед по группе {n}",
                            "в группе: " + (", ".join(mine) or "никого") if own in groups_of else "не зарегистрирован"))
