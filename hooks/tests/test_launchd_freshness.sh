...
grep -q 'все живы и свежи' <<< "$OUT" && ok || bad "T1b" "нет строки об исправном состоянии: $OUT"
...
grep -q 'выгружено: com.claudsoul.knowledge-audit' <<< "$OUT" && ok \
    || bad "T2b" "выгруженное задание не названо"
grep -q 'выгружено: com.claudsoul.scanner' <<< "$OUT" && bad "T2c" "живое задание названо выгруженным" || ok
...
grep -q 'протухло:   com.claudsoul.scanner' <<< "$OUT" && ok || bad "T3b" "протухшее не названо: $OUT"
touch "$TMP/state/last-scan-timestamp"

# --- T4: артефакта нет вовсе — отличается от «протух» ---
rm -f "$TMP/bridges/health-2026-07.md"
OUT=$(run)
grep -q 'без следа:  com.claudsoul.bridge-health' <<< "$OUT" && ok \
...
if command -v launchctl >/dev/null 2>&1; then
    _old=$(bash -c 'set -uo pipefail; launchctl list 2>/dev/null | grep -q "[[:space:]]com.claudsoul.knowledge-audit$"; echo $?' 2>/dev/null || echo skip)  # sigpipe-ok: прежняя конструкция воспроизводится намеренно, она и есть предмет проверки
    _new=$(bash -c 'set -uo pipefail; L=$(launchctl list 2>/dev/null || true); printf "%s\n" "$L" | grep -q "[[:space:]]com.claudsoul.knowledge-audit$"; echo $?' 2>/dev/null || echo skip)  # sigpipe-ok: контраст «живая труба ↔ снимок в переменную» — предмет T5
...
if grep -qE 'launchctl[^|]*\|[[:space:]]*grep' <<< "$_code"; then
