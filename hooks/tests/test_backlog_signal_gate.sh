...
grep -Fq "📋 BACKLOG D18" <<< "$OUT" && ok || bad "T1a" "открытый D18 не сигналит"
grep -Fq "📋 BACKLOG D16/D17" <<< "$OUT" && bad "T1b" "закрытый D16/D17 сигналит" || ok
grep -Fq "📋 BACKLOG D19" <<< "$OUT" && bad "T1c" "закрытый D19 сигналит" || ok
...
grep -Fq "📋 BACKLOG D18" <<< "$OUT2" && bad "T2" "закрытый D18 сигналит" || ok

# --- T3: «в работе» (◐) — открыт --------------------------------------------------
B3="$TMP/backlog-wip.md"
printf -- '- \xe2\x97\x90 **D18** взят в работу\n' > "$B3"
OUT3=$(run_collector "$B3")
grep -Fq "📋 BACKLOG D18" <<< "$OUT3" && ok || bad "T3" "◐ D18 (в работе) не считается открытым"
