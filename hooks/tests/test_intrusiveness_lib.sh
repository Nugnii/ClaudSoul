...
# Метка времени «N суток назад» в UTC. Ключей `-j -v-45d` нет у GNU date: на Linux вызов
# печатал «invalid option -- 'j'» и возвращал пустую строку, из-за чего в состояние
# записывалось пустое created_at — файл выглядел неразбираемым, очистка его пропускала,
# и тест обвинял очистку в том, чего не делал он сам. Считаем эпоху арифметикой, затем
# форматируем: сначала GNU (`-d @эпоха`), потом BSD (`-r эпоха`).
iso_days_ago() {
...
# Та же фраза с заглавной. Все остальные входы классификатора набраны строчными, поэтому
# регистр кириллицы не проверялся ни разу — а под локалью C регистронезависимое сравнение
# GNU grep складывает только ASCII, и реплика с начала предложения теряла состояние focus.
# Единственный сигнал здесь — заглавная «Не отвлекай»: «я в работе» намеренно убрано,
# иначе строчное вхождение совпало бы и без складывания регистра, и проверка была бы слепой.
SC=$(itr_compute_state "" "Не отвлекай меня сейчас")
assert_eq "state: focus phrase capitalised → focus" "focus" "$(_parse_state "$SC")"
...
# --- D87: речевые сигналы читаются по собственной речи собеседника ---
# «ещё раз» внутри пересланного документа — не признак застревания. Живой случай
# 2026-08-26: цитата внешней рецензии дала state=stuck, а stuck входит в 4D gate.
# Объём доведён до реалистичного: признак документа гасит реплику только при теле от 300
# непробельных символов — иначе гасла бы своя короткая жалоба с приложенным размеченным
# куском (раунд 1 атака 3 t6, раунд 2 атака 5 t3, два независимых критика об одном).
PASTED="# Внешняя рецензия

## Раздел 7

В большинстве проектов reflection означает: попросить модель подумать ещё раз, и дальше
автор разбирает это подробно на несколько абзацев подряд, как в настоящей пересылке.
$(for _i in 1 2 3 4 5 6; do printf 'Ещё абзац пересланного разбора, каких тут десятки.\n'; done)

| поле | значение |
|------|----------|"
SC=$(itr_compute_state "" "$PASTED")
if [ "$(_parse_state "$SC")" = "stuck" ]; then
    FAIL=$((FAIL + 1)); FAILED_TESTS+=("state: вставленный документ не должен давать stuck")
    echo "FAIL [D87 pasted document → not stuck]: получено $SC"
else PASS=$((PASS + 1)); fi

# Отрицательный контроль: та же фраза в прямой речи по-прежнему даёт stuck
SC=$(itr_compute_state "" "опять не выходит, давай ещё раз")
assert_eq "state: D87 прямая речь с «ещё раз» → stuck" "stuck" "$(_parse_state "$SC")"
