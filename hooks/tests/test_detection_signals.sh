...
# --- heredoc_body_regex: правки через оболочку видны блокеру (D79) ---
# Замер 2026-08-26: 45% правок файлов идут командами оболочки (1357 против 1675
# файловыми инструментами), а blocker-tier знания слушали только Edit/Write — то есть
# на половине правок были выключены по построению. Ключевое требование к матчеру:
# отличать ЗАПИСЬ от ПОИСКА по тому же образцу, иначе он загорится на каждом grep.
hd_eval() { # команда, регулярка → 0 fire / 1 silent
    local ti node
    ti=$(jq -cn --arg c "$1" '{command:$c}')
    node=$(jq -cn --arg r "$2" '{heredoc_body_regex:$r}')
    ds_evaluate_node "$node" "Bash" "$ti" "" 2>/dev/null
}
hd_eval "$(printf 'cat > hooks/x.sh <<%sEOF%s\ndate -j -f fmt\nEOF' "'" "'")" "date -j"
assert_eq "heredoc: запись через тело ловится" 0 $?
hd_eval 'grep -rn "date -j" hooks/' "date -j"
assert_eq "heredoc: поиск по тому же образцу молчит" 1 $?
hd_eval "$(printf 'cat > a.sh <<EOF\necho ok\nEOF')" "date -j"
assert_eq "heredoc: тело без образца молчит" 1 $?
hd_eval "$(printf 'python3 - <<%sPY%s\nimport os\nPY' "'" "'")" "import os"
assert_eq "heredoc: кавычки вокруг метки не мешают" 0 $?
hd_eval 'echo "no heredoc here"' "date -j"
assert_eq "heredoc: команда без тела молчит" 1 $?

# --- Report ---
# ── command_uses: «использую» против «упоминаю» (2026-08-28) ──────────────────
# Повод измерен на себе. Ветвь сигналов по Bash у pattern-shell-portability сработала на
# первом же живом вызове — но команда лишь СОДЕРЖАЛА слово timeout внутри кавычек пробы,
# а не запускала его. Подстрока не различает исполнение и упоминание, и на этом ломается
# любой стопор: собеседник получает вопрос там, где ничего не происходит.
#
# Правило: убрать закавыченные куски (это аргументы-строки), и смотреть, что осталось.
# Тело heredoc НЕ трогаем — код, записываемый в файл, будет исполняться, это тоже
# использование; для него есть отдельный предикат heredoc_body_regex (D79).
CU='{"command_uses":"timeout "}'
assert_eq "command_uses: запуск команды — совпадение" match \
  "$(run_node "$CU" Bash '{"command":"timeout 60 bash x.sh"}' '')"
assert_eq "command_uses: слово в одинарных кавычках — не совпадение" nomatch \
  "$(run_node "$CU" Bash "{\"command\":\"printf '%s' 'timeout is missing'\"}" '')"
assert_eq "command_uses: слово в двойных кавычках — не совпадение" nomatch \
  "$(run_node "$CU" Bash '{"command":"grep \"timeout \" file.txt"}' '')"
assert_eq "command_uses: после конвейера — совпадение" match \
  "$(run_node "$CU" Bash '{"command":"cat f | timeout 5 head"}' '')"
assert_eq "command_uses: другой инструмент — не совпадение" nomatch \
  "$(run_node "$CU" Edit '{"file_path":"x.sh","new_string":"timeout 5"}' '')"

# sed -i как предикат: sed -i.bak корректен на обеих системах, ломается sed -i с выражением.
CU2='{"command_uses":"sed -i "}'
assert_eq "command_uses: sed -i с выражением — совпадение" match \
  "$(run_node "$CU2" Bash "{\"command\":\"sed -i 's/a/b/' f\"}" '')"
assert_eq "command_uses: sed -i.bak — не совпадение" nomatch \
  "$(run_node "$CU2" Bash "{\"command\":\"sed -i.bak 's/a/b/' f\"}" '')"

echo
# ---------------------------------------------------------------------------
# shell_regex / raw_command_regex — вид команды выбирает ПРЕДИКАТ, а не библиотека.
# Повод (2026-08-28, замер по 2342 прошлым вызовам): единая нормализация дала 32 ложных
# отказа из 40. Снятие кавычек верно для вопроса «исполняется ли токен» и неверно для
# вопроса «правильно ли построен вызов»: у `sed -i` пустая строка — значащий аргумент.
# Тело heredoc — содержимое будущего файла, а не команда текущего вызова.
# ---------------------------------------------------------------------------
_FOR_RX='for[[:space:]]+[A-Za-z_][A-Za-z0-9_]*[[:space:]]+in[[:space:]]+[$][{]?[A-Za-z_][A-Za-z0-9_]*[}]?[[:space:]]*(;|do|$)'
NODE_SH=$(jq -nc --arg rx "$_FOR_RX" '{shell_regex:$rx}')
assert_eq "shell_regex: форма в самой команде — дефект" "match" \
    "$(run_node "$NODE_SH" Bash '{"command":"for d in $DOCS; do :; done"}' "")"
CMD_HEREDOC=$(printf 'cat > t.sh <<XEOF\nfor d in $DOCS; do :; done\nXEOF\n')
assert_eq "shell_regex: та же форма в теле heredoc — запись, не вызов" "nomatch" \
    "$(run_node "$NODE_SH" Bash "$(jq -nc --arg c "$CMD_HEREDOC" '{command:$c}')" "")"

_SED_RX='(^|[;&|(])[[:space:]]*sed[[:space:]]+-i[[:space:]]+[^'"'"'"[:space:]]'
NODE_SED=$(jq -nc --arg rx "$_SED_RX" '{raw_command_regex:$rx}')
assert_eq "raw_command_regex: sed -i со скриптом — падает здесь" "match" \
    "$(run_node "$NODE_SED" Bash '{"command":"sed -i s/a/b/ f.txt"}' "")"
assert_eq "raw_command_regex: sed -i с пустой строкой — верно на BSD" "nomatch" \
    "$(run_node "$NODE_SED" Bash "$(jq -nc --arg c "sed -i '"'"''"'"' s/a/b/ f.txt" '{command:$c}')" "")"
assert_eq "raw_command_regex: sed -i.bak — верно на обеих" "nomatch" \
    "$(run_node "$NODE_SED" Bash '{"command":"sed -i.bak s/a/b/ f.txt"}' "")"

CMD_ABS_HD=$(printf 'cat > t.sh <<XEOF\nclaudsoul-nonexistent-binary 5 ls\nXEOF\n')
assert_eq "command_absent: имя в теле heredoc — не вызов" "nomatch" \
    "$(run_node '{"command_absent":"claudsoul-nonexistent-binary"}' Bash "$(jq -nc --arg c "$CMD_ABS_HD" '{command:$c}')" "")"
