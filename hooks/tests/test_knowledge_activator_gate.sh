...
run() { # $1=команда для tool_input
    printf '{"session_id":"%s","cwd":"%s","tool_name":"Bash","tool_input":{"command":"%s"}}' \
      ...
      | HOME="$FAKE_HOME" CLAUDE_CODE_SESSION_ID="$SID" STATE_DIR="$TMP/state" SKIP_MCP_FALLBACK=1 \
        bash "$HOOK" >/dev/null 2>"$TMP/hook-err"
    local rc=$?
    # run зовётся напрямую (не через $(…)) — счётчик доживает до итога. Хук обязан
    # выходить 0; смерть раньше была неотличима от «гейт молчит» (D220).
    [ "$rc" -eq 0 ] || bad "run" "хук умер: rc=$rc, stderr: $(tail -c 200 "$TMP/hook-err" 2>/dev/null)"
}
