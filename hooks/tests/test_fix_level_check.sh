...
# --- T13-T16: второй словарь — речевые outcome_signals знаний (D231) ---
LESSONS="$TMP/lessons"; mkdir -p "$LESSONS"
cat > "$LESSONS/pattern-os-probe.md" << 'KF'
---
name: os-probe
type: pattern
confidence: 3
...
status: active
---
Фикстура: нарушение звучит как «запущено и ждём».
KF
cat > "$LESSONS/pattern-os-empty.md" << 'KF'
---
name: os-empty
type: pattern
outcome_signals: []
status: active
---
Пустой список — фраз не даёт.
KF

# T13: фраза нарушения в ответе → pending с классом и именем знания
make_transcript "$TMP/t13.jsonl" "статус?" "Всё запущено и ждём результата."
OUT=$(LESSONS_DIR="$LESSONS" run_hook "Stop" "sid-t13" "$TMP/t13.jsonl")
assert_file_contains "$STATE_DIR/fix-level-sid-t13.jsonl" '"class":"outcome_signal"' "T13: класс"
assert_file_contains "$STATE_DIR/fix-level-sid-t13.jsonl" '"knowledge":"pattern-os-probe"' "T13: имя знания"

# T14: surface несёт блок ⛳ с именем знания и фразой
OUT=$(LESSONS_DIR="$LESSONS" run_hook "UserPromptSubmit" "sid-t13" "")
assert_contains "$OUT" "Outcome-signal" "T14: блок нарушений"
assert_contains "$OUT" "pattern-os-probe" "T14: имя знания в сообщении"
assert_contains "$OUT" "запущено и ждём" "T14: фраза в сообщении"

# T15: ответ без фразы → pending не создаётся
make_transcript "$TMP/t15.jsonl" "статус?" "Прогон завершён, вот результат: 3 из 3."
OUT=$(LESSONS_DIR="$LESSONS" run_hook "Stop" "sid-t15" "$TMP/t15.jsonl")
assert_file_missing "$STATE_DIR/fix-level-sid-t15.jsonl" "T15: тишина без фразы"

# T16: повторный скан того же ответа не плодит дублей
OUT=$(LESSONS_DIR="$LESSONS" run_hook "Stop" "sid-t13" "$TMP/t13.jsonl")
N_OS=$(grep -c '"class":"outcome_signal"' "$STATE_DIR/fix-level-sid-t13.jsonl" 2>/dev/null || printf '0')
if [ "$N_OS" = "1" ]; then PASS=$((PASS + 1))
else FAIL=$((FAIL + 1)); echo "FAIL [T16]: дубли outcome_signal: $N_OS"; fi
