...
# T23: строка-носитель версии изменилась, а версия та же → это НЕ бамп.
#
# Живой случай 21.08.2026. Генератор count-stats обновил в README строку
# «**Current version:** v1.27.0 … 49 active hooks» на «… 50 active hooks».
# Номер версии в ней не менялся — менялось число рядом. Страж прочитал маркер
# в добавленной строке и потребовал обновить пять документов под релиз, которого
# не было.
#
# Класс тот же, что этот файл чинил дважды: совпадение с ТЕКСТОМ вместо совпадения
# со СМЫСЛОМ. Признак «строка с версией изменилась» — не признак «версия изменилась»;
# различает их только сравнение с удалённой строкой.
REPO23="$TMP/repo23"
seed_versioned "$REPO23" "1.27.0"
printf '# Readme v1.27.0\n\n**Current version:** v1.27.0 — 49 active hooks\n' > "$REPO23/README.md"
git -C "$REPO23" add . >/dev/null 2>&1
git -C "$REPO23" commit -q -m "readme со счётчиком" 2>/dev/null
# Меняется ТОЛЬКО число хуков в той же строке.
printf '# Readme v1.27.0\n\n**Current version:** v1.27.0 — 50 active hooks\n' > "$REPO23/README.md"
git -C "$REPO23" add . >/dev/null 2>&1
OUT=$(run_with "sid23" "$REPO23" "git commit -m 'счётчик хуков'")
assert_empty "$OUT" "T23: версия та же, изменилось число рядом → не бамп"

# T24: контрольный случай к T23 — на том же дереве настоящий бамп обязан гореть.
# Без него зелёный T23 неотличим от стража, ослеплённого до немоты.
REPO24="$TMP/repo24"
seed_versioned "$REPO24" "1.27.0"
printf '# Readme v1.27.0\n\n**Current version:** v1.27.0 — 49 active hooks\n' > "$REPO24/README.md"
git -C "$REPO24" add . >/dev/null 2>&1
git -C "$REPO24" commit -q -m "readme со счётчиком" 2>/dev/null
printf '# Readme v1.28.0\n\n**Current version:** v1.28.0 — 50 active hooks\n' > "$REPO24/README.md"
git -C "$REPO24" add . >/dev/null 2>&1
OUT=$(run_with "sid24" "$REPO24" "git commit -m release")
assert_contains "$OUT" "Docs family drift" "T24: настоящий бамп на том же дереве → fire"
assert_contains "$OUT" "1.28.0" "T24b: назван именно НОВЫЙ номер, не старый"

# ============================================================================
# T25-T28 (D67): автотаблицы сверяются, когда меняются их ИСТОЧНИКИ, а не только
# когда выпускают релиз.
#
# Коммит 771e7fc (21.08) внёс расхождение рядовой правкой шапки хука: генератор
# берёт из шапки ТОЛЬКО строку 2, а описание было разбито надвое. Бампа версии в
# том коммите не было, проверка таблиц сидела внутри Path A — страж промолчал.
# Расхождение прожило сутки и вскрылось случайно, регенерацией по другому поводу.
# ============================================================================
seed_tables() {   # репозиторий с рабочим генератором таблиц и одним хуком-источником
    local r="$1"
    rm -rf "$r"; mkdir -p "$r/hooks" "$r/scripts" "$r/docs"
    git -C "$r" init -q 2>/dev/null
    git -C "$r" config user.email "t@e"; git -C "$r" config user.name "t"
    printf '#!/usr/bin/env bash\n# foo.sh — описание в одну строку.\n' > "$r/hooks/foo.sh"
    # Генератор: вторая строка каждого хука → тело блока. Уважает README_FILE,
    # как настоящий scripts/regen-readme-skills.sh.
    cat > "$r/scripts/regen-readme-skills.sh" <<'GEN'
#!/usr/bin/env bash
ROOT="${1:-.}"
F="${README_FILE:-$ROOT/README.md}"
BODY=$(for h in "$ROOT"/hooks/*.sh; do sed -n '2p' "$h"; done)
awk -v body="$BODY" '
  /HOOKS-TABLE:START/ { print; print body; skip = 1; next }
  /HOOKS-TABLE:END/   { skip = 0 }
  !skip               { print }
' "$F" > "$F.new" && mv "$F.new" "$F"
GEN
    printf '# Readme v1.0.0\n\n<!-- HOOKS-TABLE:START -->\n<!-- HOOKS-TABLE:END -->\n' > "$r/README.md"
    README_FILE="$r/README.md" bash "$r/scripts/regen-readme-skills.sh" "$r"
    printf '# Plan\n' > "$r/PLAN.md"; printf '# Claude\n' > "$r/CLAUDE.md"
    printf '# Changelog\n' > "$r/CHANGELOG.md"; printf '# Arch\n' > "$r/docs/architecture.md"
    git -C "$r" add . >/dev/null 2>&1
    git -C "$r" commit -q -m seed 2>/dev/null
}

# T25: шапка хука изменилась, таблица отстала, версия НЕ менялась → страж обязан сказать.
REPO25="$TMP/repo25"
seed_tables "$REPO25"
printf '#!/usr/bin/env bash\n# foo.sh — описание стало другим.\n' > "$REPO25/hooks/foo.sh"
git -C "$REPO25" add . >/dev/null 2>&1
OUT=$(run_with "sid25" "$REPO25" "git commit -m 'правка шапки хука'")
assert_contains "$OUT" "автотаблицы устарели" "T25: источник изменён, таблица отстала, версии нет → fire"

# T26: контрольный случай к T25 — таблицы согласованы, всё прочее то же → молчит.
# Без него зелёный T25 неотличим от стража, который горит на любой правке хука.
REPO26="$TMP/repo26"
seed_tables "$REPO26"
printf '#!/usr/bin/env bash\n# foo.sh — описание стало другим.\n' > "$REPO26/hooks/foo.sh"
README_FILE="$REPO26/README.md" bash "$REPO26/scripts/regen-readme-skills.sh" "$REPO26"
git -C "$REPO26" add . >/dev/null 2>&1
OUT=$(run_with "sid26" "$REPO26" "git commit -m 'правка шапки + регенерация'")
assert_empty "$OUT" "T26: таблицы согласованы → молчит"

# T27: рассинхрон есть, но источников таблиц в коммите нет → не наш повод.
# Иначе страж загорался бы на каждом коммите подряд, пока кто-нибудь не regen-ит.
REPO27="$TMP/repo27"
seed_tables "$REPO27"
printf '#!/usr/bin/env bash\n# foo.sh — описание стало другим.\n' > "$REPO27/hooks/foo.sh"
git -C "$REPO27" add . >/dev/null 2>&1
git -C "$REPO27" commit -q -m "рассинхрон уехал" 2>/dev/null
printf 'заметка\n' > "$REPO27/docs/notes.md"
git -C "$REPO27" add . >/dev/null 2>&1
OUT=$(run_with "sid27" "$REPO27" "git commit -m 'посторонняя правка'")
assert_empty "$OUT" "T27: источники таблиц не тронуты → молчит"

# T28: тот же набор отсутствующих не повторяется в одной сессии (throttle).
OUT=$(run_with "sid25" "$REPO25" "git commit -m 'та же правка ещё раз'")
assert_empty "$OUT" "T28: повтор в той же сессии заглушен throttle"

echo ""
echo "=================================="

# ============================================================================
# T23: автотаблица живёт НЕ в README — страж находит её по маркеру (D212, 30 августа 2026)
#
# Список `README.md README.ru.md` был зашит в страж, а таблица хуков к тому времени жила в
# docs/reference.md и docs/reference.ru.md; страж молчал на устаревшей таблице, потому что
# смотрел не в тот носитель (тот же класс, что D105). Документы с маркерами теперь
# находятся по дереву git, а не перечисляются.
# ============================================================================
REPO23="$TMP/repo23"
seed_versioned "$REPO23" "1.0.0"
mkdir -p "$REPO23/scripts" "$REPO23/hooks" "$REPO23/skills/demo"
cp "$(cd "$(dirname "$0")/../.." && pwd)/scripts/regen-readme-skills.sh" "$REPO23/scripts/"
printf '#!/usr/bin/env bash\n# demo-hook.sh — демонстрационный хук для теста стража.\n# en: demo hook for the guard test.\n' > "$REPO23/hooks/demo-hook.sh"
printf -- '---\nname: demo\ndescription: демонстрационный скилл\n---\n' > "$REPO23/skills/demo/SKILL.md"
printf '# Reference\n\n<!-- HOOKS-TABLE:START -->\nстарое\n<!-- HOOKS-TABLE:END -->\n' > "$REPO23/docs/reference.md"
git -C "$REPO23" add . >/dev/null 2>&1
git -C "$REPO23" commit -q -m "reference с устаревшей таблицей" 2>/dev/null
printf '# Changelog\n\n## [1.1.0] - 2026-08-30\n' > "$REPO23/CHANGELOG.md"
printf '# Readme v1.1.0\n' > "$REPO23/README.md"
printf '# Claude\n\nправка\n' > "$REPO23/CLAUDE.md"
printf '# Plan\n\n| Текущая версия | v1.1.0 |\n' > "$REPO23/PLAN.md"
printf '# Arch\n\n> Покрывает через v1.1.0\n' > "$REPO23/docs/architecture.md"
git -C "$REPO23" add . >/dev/null 2>&1
OUT=$(run_with "sid23" "$REPO23" "git commit -m release")
assert_contains "$OUT" "docs/reference.md (автотаблицы устарели" "T23: устаревшая автотаблица вне README найдена по маркеру"
# КОНТРПРИМЕР: та же таблица, но свежая (перегенерирована) → страж молчит
README_FILE="$REPO23/docs/reference.md" bash "$REPO23/scripts/regen-readme-skills.sh" "$REPO23" >/dev/null 2>&1
git -C "$REPO23" add . >/dev/null 2>&1
OUT=$(run_with "sid23b" "$REPO23" "git commit -m release")
assert_empty "$OUT" "T23b: таблица перегенерирована → молчит"
