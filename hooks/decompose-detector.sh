#!/usr/bin/env bash
# decompose-detector.sh — pre-execution router в UserPromptSubmit: шаги→/decompose, решения→/грилинг.
# en: UserPromptSubmit: pre-execution router — suggests /decompose for multi-step requests, /грилинг for open-decision/vague ones.
...
#   РЕШЕНИЯ (→/грилинг): маркеры выбора и дизайна — либо, как лучше, какой вариант,
...
#   решений ≥2 И решений > шагов → /грилинг; иначе шагов ≥4 → /decompose; иначе тихо.
...
#   - уже упомянут любой из инструментов (decompose/разбей/грилинг/грил/хейтер/прожар): skip
...
case "$USER_LOWER" in
    ...
    *"/грилинг"*|*"грилинг"*|*"Грилинг"*|*"грил "*|*"хейтер"*|*"Хейтер"*|*"прожар"*|*"Прожар"*) exit 0 ;;
esac
...
# -----------------------------------------------------------------------
# Decision-counting (→/грилинг): маркеры открытых решений и дизайна.
...
if [ "$ROUTE" = grill ]; then
    MESSAGE="🧠 Открытые решения ($DECISION_COUNT сигналов, порог $DECISION_THRESHOLD; шагов $STEP_COUNT) — рекомендую /грилинг.

Задача пахнет неразрешёнными решениями, а не готовым списком шагов. /грилинг обойдёт дерево решений раундами (каждый вопрос с рекомендованным ответом), вытащит молчаливые допущения и доведёт до общего понимания — ДО плана и кода.

Если решения на деле уже улажены — проигнорируй. Готовый артефакт критиковать — это /хейтер, не грилинг."
else
