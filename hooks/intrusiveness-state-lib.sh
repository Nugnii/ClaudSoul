...
# shellcheck source=/dev/null
# user_own_speech нужен классификатору: речевые сигналы читаются по собственной речи
# собеседника, а не по вставленному документу (BACKLOG D87). Без библиотеки классификатор
# работает по-прежнему — там стоит проверка через command -v.
_ITR_INPUT_LIB="${BASH_SOURCE[0]%/*}/hook-input-lib.sh"
[ -f "$_ITR_INPUT_LIB" ] && source "$_ITR_INPUT_LIB"
