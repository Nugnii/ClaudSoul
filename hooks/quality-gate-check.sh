...
check_contract() {
    ...
    if [ -z "$type_line" ]; then
        add "нет **Type:**"
    elif ! printf '%s' "$type_line" | grep -qE '\*\*Type:\*\*[[:space:]]*(worker|coordinator|orchestrator|reference)'; then
    ...
    if ! printf '%s' "$type_line" | grep -q 'reference'; then
        local dod
        dod=$(printf '%s\n' "$staged" | awk '
        ...
        elif ! printf '%s\n' "$dod" | grep -qE '^[[:space:]]*-[[:space:]]*\[[ xX]\]'; then
    ...
    printf '%s\n' "$staged" | grep -q '^\*\*Changes:\*\*' && add "секция **Changes:** запрещена контрактом"
