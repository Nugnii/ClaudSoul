#!/usr/bin/env bash
# itr-event-detector.sh — UserPromptSubmit: находит мягкое предложение в прошлом ответе агента, определяет, принял его собеседник или проигнорировал, и записывает исход — иначе бюджеты слоя 6 не набираются.
# en: UserPromptSubmit: finds a gentle suggestion in the assistant's prior turn, classifies whether the user accepted or ignored it, and logs the outcome — without this the L6 budgets never accumulate.
#
...
if [ -n "$LAST_PARA" ] && grep -qF '?' <<< "$LAST_PARA"; then
    for marker in "${GENTLE_MARKERS[@]}"; do
        if grep -qF "$marker" <<< "$LAST_PARA_LOWER"; then
            GENTLE_MATCH="$marker"
            break
        fi
    done
