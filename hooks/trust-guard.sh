...
match_signature() {
    ...
    if echo "$exec_norm" | grep -qE '(^|[[:space:];&|])rm[[:space:]]+(-[rRfv]*[rR][rRfv]*[fF][rRfv]*|-[fF][rRfv]*[rR]|--recursive.*--force|--force.*--recursive|-r[[:space:]]+-f|-f[[:space:]]+-r)'; then
        SIGNATURE="rm -rf"
        TARGET=$(echo "$norm" | sed -nE 's/.*rm[[:space:]]+(-[-a-zA-Z]+[[:space:]]+)+([^[:space:]]+).*/\2/p' | head -1)
        return 0
    fi

    if echo "$exec_norm" | grep -qE '(^|[[:space:];&|`])git[[:space:]]+reset[[:space:]]+--hard'; then
    ...
    if echo "$exec_norm" | grep -qE '(^|[[:space:];&|`])git[[:space:]]+push[[:space:]]+.*(-f($|[[:space:]])|--force)'; then
    ...
    if echo "$exec_norm" | grep -qE '(^|[[:space:];&|`])git[[:space:]]+branch[[:space:]]+-D([[:space:]]|$)'; then
        SIGNATURE="git branch -D"
        TARGET=$(echo "$norm" | sed -nE 's/.*git[[:space:]]+branch[[:space:]]+-D[[:space:]]+([^[:space:];&|]+).*/\1/p' | head -1)
        return 0
    fi

    if echo "$exec_norm" | grep -qE '(^|[[:space:];&|`])git[[:space:]]+checkout[[:space:]]+--[[:space:]]'; then
        SIGNATURE="git checkout --"
        TARGET=$(echo "$norm" | sed -nE 's/.*git[[:space:]]+checkout[[:space:]]+--[[:space:]]+([^[:space:];&|]+).*/\1/p' | head -1)
        return 0
    fi

    if echo "$exec_norm" | grep -qE '(^|[[:space:];&|`])git[[:space:]]+clean[[:space:]]+[-a-zA-Z]*f'; then
    ...
    if echo "$exec_norm" | grep -qE '(^|[[:space:];&|])rm[[:space:]]+-[fF][[:space:]]+(/|~|\$HOME)'; then
...
if [ -n "$TRANSCRIPT_PATH" ] && [ -f "$TRANSCRIPT_PATH" ]; then
    ...
    if [ -n "$LAST_USER_TEXTS" ]; then
        ...
        USER_LOWER=$(to_lower "$LAST_USER_TEXTS")
