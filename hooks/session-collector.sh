...
if [ -f "$DIS_LIB" ]; then
    ...
    if [ -n "$OPEN_RECORDS" ]; then
        ...
        PENDING_DETAIL=$(printf '%s\n' "$OPEN_RECORDS" | while IFS='|' read -r s k d c t; do
            ...
            printf '      не к месту→ только запись исхода, счётчики не трогать (сессия %s)\n' "${s:0:8}"
        done)
    fi
fi
...
if [ "${PENDING_COUNT:-0}" -gt 0 ] 2>/dev/null; then
    ...
    ALERTS="${ALERTS}⚡ ${PENDING_COUNT} blocker-tier знание(й) без исхода — подтвердилось / устарело. "
fi
...
if [ -f "$MEASURE_DUE_SH" ]; then
    MEASURE_OVERDUE=$(bash "$MEASURE_DUE_SH" check 2>/dev/null | grep -c '^  просрочен:' || true)
    : "${MEASURE_OVERDUE:=0}"
    if [ "${MEASURE_OVERDUE:-0}" -gt 0 ] 2>/dev/null; then
        ALERTS="${ALERTS}📏 ${MEASURE_OVERDUE} просроченных замер(ов) — scripts/measurement-due.sh. "
    fi
fi
