...
# ВНИМАНИЕ (замер 27 августа 2026, D90): у этой функции НОЛЬ вызывающих во всём
# репозитории. Формула `weight = source_factor × domain_factor` описана в трёх местах
# (knowledge/META.md, skills/retro/references/promotion-rules.md, CHANGELOG) и
# исполняется только агентом вручную по тексту правила, то есть уровнем 1. Пока
# вызывающих нет, любая правка внутри неё декоративна — включая обещанный `fragile ×1.2`.
# Прежде чем дописывать сюда, надо решить, где взвешивание применяется на деле:
# кандидат — `knowledge-counter-bump.sh` в ветке `contradicted`, но это делает
# `contradicted_count` дробным, а его читают как целое `reliability` и формула priority.
