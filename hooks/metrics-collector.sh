...
if [ "$ITR_SESSIONS_TOTAL" -gt 0 ] && command -v jq >/dev/null 2>&1; then
    ...
    # awk, а не head: `tail -40 | head -20` под set -euo pipefail — гонка. head закрывает
    # трубу после 20 строк, tail ловит EPIPE на 21-й и умирает с «write error: Broken
    # pipe», хук гибнет до записи metrics.md. Буфер трубы обычно съедает все 40 строк
    # раньше — потому локально зелено, а на медленном CI флак (прогоны 33330392685,
    # 33337205730; лицо дал rc+stderr из _run_collector). awk читает вход до EOF —
    # EPIPE не рождается. Класс шире этого места — D225.
    PREV20=""
    if [ "$ITR_SESSIONS_TOTAL" -gt 20 ]; then
        PREV20=$(printf '%s\n' "$ITR_SESSIONS_JSONL" | tail -40 | awk 'NR<=20' | _itr_aggregate)
    fi
