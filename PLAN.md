...
| Поле | Значение |
|------|----------|
| Текущая версия | **v1.32.0** (2026-08-31) |
...
Разрыв: агент себе противоречит, не замечает до коррекции собеседником. Сигнал
`cascading.backward_count` этим разрывом не является: он считает поправки собеседника,
...
| **H4** | FSRS-decay предотвратит засорение базы | 9 | 🔁 | % weakened знаний / 30 календарных дней (окно наблюдения; сам decay — в днях опыта, D234) |
...
| **H23** | `/enrich` post-ingest увеличивает среднюю плотность entity | v1.5.7 | ⏳ | attrs/entity по active-записям, dormant вне знаменателя (baseline от v1.5.7) |
...
Единственный реестр — [docs/decisions.md](docs/decisions.md), append-only. Ранние
решения (Markdown как хранилище, три уровня, confidence 1-5, глобальные знания +
проектные кейсы, два контура, L6 как 4D gate) — там же, раздел «Ранние решения»,
R1–R6; здесь реестр не ведётся.
...
3. **Roadmap не ведётся.** Таблица фаз — в архиве; детали релизов → CHANGELOG. План не дублирует историю.
