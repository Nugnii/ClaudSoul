...
├── hooks/                 # 52 активных хука + 26 библиотек alive learning system + tests/
...
│   └── *.md               # 9 принципов + 22 universal-паттерна (генерируется scripts/regen-seed.py)
...
└── hooks/tests/           # 137 файлов тестов хуков + 24 mcp (см. секцию 5)
```
...
> Состояние на **v1.28.1** (2026-08-26). Это документ состояния: «как есть сейчас». Что менялось и когда — `CHANGELOG.md`;
> долг — `BACKLOG.md`; состояние мостов — `bridges/_index.md`. Хронику сюда не писать.

| Компонент | Состояние |
|-----------|-----------|
| Когнитивные слои 1-7 + каскадность | Реализованы |
| Хуки alive learning system | 52 активных + 26 библиотек |
...
| Тесты | 137 файлов тестов хуков + 24 mcp (121 mcp-тест); зелёные на macOS и в чистом контейнере Linux; drift-check по 7 парам |
