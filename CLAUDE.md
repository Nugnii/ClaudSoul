...
Когнитивная модель из 7 слоёв (слои 1-6 несут собственный код; слой 7 наблюдается
в данных, но своего кода не имеет — см. секцию 5 и таблицу слоёв в README):
...
| Метаданные знаний | YAML frontmatter по `knowledge/META.md` (confidence, impact, 9 якорей, demand) |
| Skills API | Claude Code native |
| Семантический поиск | сервер MCP (sqlite-vec + fastembed) |

## 3. Структура

Выборочно: показано то, на что ссылается этот документ; полный список — `git ls-files`.

```
ClaudSoul/
├── CLAUDE.md              # Этот файл
├── README.md              # Документация; README.ru.md — русская версия
├── PLAN.md                # План: что впереди, гипотезы
├── CHANGELOG.md           # История изменений; внутренний архив (не публикуется) — вынесенная хроника
├── VERSION                # Версия проекта, источник истины
...
├── uninstall.sh           # Снятие своего по сверке с репозиторием
├── bin/                   # claudsoul.js (канал npm, ADR-009), resolve-claudsoul-repo.sh
├── lib/                   # claude-md-merge.sh — слияние правил в проектный CLAUDE.md
├── launchd/               # plist автосканера, weekly knowledge-audit, monthly bridge-health
├── mcp-server/            # сервер MCP: server.py (9 tools), indexer.py, storage.py, brain.py, visualization/, tests/
├── .claude-docs/          # modules/ (доки модулей), specs/, dep-index.tsv, narrative и журнал активности
├── docs/
│   ├── architecture.md    # Когнитивная архитектура (7 слоёв)
│   ├── research.md        # Результаты исследований
│   └── decisions.md       # Архитектурные решения (ADR)
├── hooks/                 # 63 активных хука + 28 библиотек alive learning system + tests/
...
│   ├── META.md            # Мета-правила эволюции знаний и формат записи
...
└── hooks/tests/           # 295 файлов тестов хуков + 24 mcp (см. секцию 5)
```
...
> Состояние на **v1.32.0** (2026-08-31). Это документ состояния: «как есть сейчас». Что менялось и когда — `CHANGELOG.md`;
...
| Хуки alive learning system | 63 активных + 28 библиотек |
| Скиллы | 23: user-invocable + координатор + оркестратор |
| Знания, operational-контур | case/pattern/principle: 9 якорей, demand, edges, FSRS decay (прошедшее — дни опыта: сессии/темп, календарь фоллбеком), кросс-доменный перенос; seed генерируется `regen-seed.py` |
...
| Публикация наружу | снимок из git archive → exclude → replace → redact → согласование производных чисел → числа документов против мира (реестр `scripts/doc-claims.tsv`) → возраст описаний против механизмов (`scripts/docs-staleness.sh`) → гейт по выходу (запреты + pytest по копии снимка); способ выбирается замером истории публичного репозитория: чистая → force-push (issues и звёзды живут), грязная → пересоздание |
...
| Долг | BACKLOG ClaudSoul (кросс-проектно) + локальный BACKLOG.md инициированного проекта (по cwd); архив не считается. Цикл проблемы: повод разбора → пункт с измеримым результатом и командой проверки → сверка при починке (`backlog-touch-check`) → закрытие с показанием → completion-gate на Stop прогоняет «Проверку» ☑-пунктов, красная называется → архив (☑ без «Проверки» или с красной «Проверкой» и ⊘ без «Вердикта» не переносятся) |
| Session Registry | lifecycle, дельты, startup context |
| Автономность | автосканер, weekly knowledge audit, monthly bridge health |
| Тесты | 295 файлов тестов хуков + 24 mcp (121 mcp-тест); зелёные на macOS и в чистом контейнере Linux; drift-check по 7 парам |
