# Модуль: ablation-runner (запускалка теневых пар)

**Назначение.** Исполняемая часть prereg-протокола `docs/ablation-protocol.md`
(D64): журнал задач с механическим правилом входа, детерминированный сэмплер с
публичной случайностью, заморозка task package, песочницы двух плеч, парный
анализ. Всё состояние — приватно в `ABLATION_DIR` (default `~/.claude/ablation`),
...
| `scripts/ablation/snapshot.sh` | заморозка пакета: repo.tar + knowledge.tar + runtime.tar + manifest c hash | §6 |
| `scripts/ablation/env-diff.sh` | манифест treatment: full/vanilla; расхождение = infrastructure_failure | §6 |
...
| `scripts/ablation/pair-analyze.py` | exact McNemar + Agresti–Min CI + правило трёх исходов; Δ_all + Δ_surface | §11 |
...
| `scripts/ablation/pair.sh` | завершение пары (бинаризация → pairs.jsonl) и аннулирование целиком с лимитом 2 повторов | §9 |
...
задачу; снимок и прогон плеча неизменяемы (повтор пары — только целиком);
