...
import re, sys, os, pathlib, datetime, subprocess
...
for b in moved:
    ...
    # Красная «Проверка» — тоже не закрытие (completion-gate, эскалация
    # pattern-completion-by-internal-proxy, confirmed 14): метку ☑ ставит исполнитель,
    # а критерий закрытия несёт сам пункт — команда прогоняется, и верится коду
    # возврата, не отчёту. Признак с одним последствием: закрытие утверждает
    # «команда даёт 0», команда даёт не-0 — утверждение ложно, переносить нечего.
    # BACKLOG_ARCHIVE_RUN_CHECKS=0 — аварийное отключение (например, команда пункта
    # зависла); таймаут на команду — BACKLOG_CHECK_TIMEOUT (по умолчанию 120 с).
    # ВСЕ строки «Проверки» пункта, и команда исполняется КАК ЕСТЬ (не склейкой):
    # первая редакция брала re.search + " ".join — вторая красная строка молча
    # проезжала за зелёной первой, а многострочная команда склеивалась в другую
    # (прожарка 31.08, атаки adv6; тот же разборщик — hooks/completion-gate.sh).
    if b[1] == "☑" and os.environ.get("BACKLOG_ARCHIVE_RUN_CHECKS", "1") != "0":
        for m in re.finditer(r'\*\*Проверка\.\*\*\s*`([^`]+)`', body):
            cmd = m.group(1).strip()
            try:
                rc = subprocess.run(["bash", "-c", cmd], capture_output=True,
                                    cwd=str(backlog.parent),
                                    timeout=int(os.environ.get("BACKLOG_CHECK_TIMEOUT", "120"))).returncode
            except Exception:
                rc = 125
            if rc != 0:
                disp = " ".join(cmd.split())
                unproven.append((iid, f"☑, а его же «Проверка» красная (rc={rc}): `{disp[:90]}`"))
                break
