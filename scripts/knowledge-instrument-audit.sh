...
# Результат: в очереди «знание → инструмент» нет пунктов старше 30 дней (KIA_QUEUE_MAX_DAYS) без вердикта, вердикты не нарушены, все поля прочитаны
# Проверка результата: bash scripts/knowledge-instrument-audit.sh даёт 0; 1 — просроченный пункт очереди, нарушенный вердикт или нечитаемое поле (сама непустая очередь код не роняет — её размер печатается)
#
#
