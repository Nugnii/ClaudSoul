...
#
# Замораживает на пару: репозиторий задачи (HEAD), базу знаний (живую — она
# часть treatment) и исполняемый runtime ClaudSoul (установленную policy).
...
jq -n --arg id "$TASK_ID" --arg ts "$TS" --arg head "$HEAD_SHA" --arg dirty "$DIRTY" \
      ...
      --arg u "$(sha "$PKG/runtime.tar")" \
      '{task_id:$id, snapshot_ts:$ts, repo_head:$head, repo_dirty_files:($dirty|tonumber),
        sha256:{repo:$r, knowledge:$k, runtime:$u}}' > "$PKG/manifest.json"
