#!/usr/bin/env bash
# checker.sh — проверяющий контракт тройки ВНЕ writable-песочниц (протокол §9).
#
...
#   run <task_id> full|core|vanilla — классификация исхода плеча:
...
register)
    ...
    ;;
run)
    ID="${1:?task_id}"; ARM="${2:?плечо: full|core|vanilla}"
    case "$ARM" in full|core|vanilla) ;; *)
        echo "checker: плечо — full|core|vanilla (получено '$ARM')" >&2; exit 1 ;; esac
