...
#
#
# Повод. `scripts/measurements.tsv` перечисляет замеры с периодом 0 — «по событию,
# по расписанию launchd»: три задания launchd, проверяемые здесь, и `usage-outcome`,
# у которого сторожа нет. `measurement-due.sh:50-52` делает `continue` ДО сравнения
...
# запаса на пропущенный запуск. Допуск — не меньше удвоенного периода: еженедельное
# 168 ч → 336 ч, ежемесячное ~720 ч → 1512 ч; у сканера (4 ч) — 12 ч, три срока, потому
# что ночной сон машины пропускает два четырёхчасовых срока подряд (см. выше).
#
