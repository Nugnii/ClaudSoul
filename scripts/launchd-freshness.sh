...
# Допуски выведены, а не назначены (D217). Прежняя формула «период + один пропуск»
# (у сканера дала бы 8 ч) для интервального задания неприменима, и это замерено
# 30 августа 2026 по scanner.log: 562 скана, медиана интервала 4.0 ч, но 93 из 561
# интервалов длиннее 8 ч (максимум 63 ч) — машина спит, StartInterval во сне не тикает,
# и восьмичасовой допуск краснел бы на ЖИВОМ задании в 17% интервалов. Отсюда:
#   интервальные задания — период + ночной простой машины (~8 ч): 4 + 8 = 12;
#   календарные — два периода, один пропущенный запуск: неделя 2×168 = 336,
#   месяц 2×31.5 дн. = 1512 (полмесяца запаса сверх двух месяцев — длинные месяцы).
