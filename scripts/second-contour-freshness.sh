...
dorm = re.compile(r"^status:\s*['\"]?dormant", re.M)

# Спящие (status: dormant) в свежести НЕ участвуют: ревизия D235 (31 августа 2026)
# пометила выведенное из употребления датой и причиной — требовать от спящего
# обновлений значило бы толкать к обогащению мёртвого, что пунктом исключено.
# Спящее считается отдельным числом, чтобы вывод из свежести оставался видимым.
buckets, no_date, valid_null, total, dormant = [], 0, 0, 0, 0
...
print(f"Второй контур: {total} записей, активных {total - dormant}, спящих {dormant}, "
      f"с датой обновления {len(buckets)}, без даты {no_date}.")
print(f"Старше {max_days} дн. (активные): {len(stale)}. Фактов/связей со сроком годности `null` (активные): {valid_null}. Спящих: {dormant}.")
if buckets:
    print("Самые давние:")
