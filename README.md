...
<sub>The author's base after four months, rendered live from <code>~/.claude/global-lessons/</code> by the MCP server's <code>open_graph</code>. 414 items in the base; the graph draws 287 of them as nodes and 1086 edges, of which 64 come from explicit relation files. A fresh install starts with 37 seed items and grows from your own sessions. (UI shown in Russian; the graph itself is language-agnostic.)</sub>
...
Sessions are counted from the L6 telemetry (`intrusiveness-history.jsonl`, 333 unique sessions over the span). The session registry counts 380 for the same period, which would make the fall steeper, not shallower — the more conservative denominator is the one used here.
...
| 7 | Co-cognition | Did the agent contribute a hypothesis, or only execute? | Emergent — 60 items recorded with `origin: co-cognition`; the disagreement loop has code (75% of 329 events resolved), the layer itself has none |
...
|---|---|---|
| Recall the rule before starting | `knowledge-activator`, 9 anchors, PreToolUse | 13,276 injections; leader `principle-verify-before-acting`, 1,114 |
| Recognise a familiar pit in advance | `blocker-tier-check` + `detection-signals-lib` | 376 pre-action markers across 239 sessions |
| Pull the hand back from a destructive command | `trust-guard`, PreToolUse[Bash] | 32 firings |
| Take a failure apart and write it down | `error-tracker` → `five-whys-gate` → `/learn` | 190 of 263 recorded cases came from a failure |
| Turn separate incidents into a rule | promotion rules, `source_cases` field | 263 cases → 37 patterns, 10 principles; lineage on 46 of 47 |
...
from success is the weaker half — 37 success cases against 190 failures. And almost nothing has ever
been retired: `contradicted_count` is non-zero on 1 item out of 414, so the decay machinery has had
...
- **`contradicted_count` has moved off zero exactly once** (1 item of 414, on 28 Aug 2026). A counter that only goes up measures nothing; the refutation loop exists in code and has a single record — the contradiction column in the table above is still closer to design than to evidence.
