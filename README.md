...
52 hooks on Claude Code lifecycle events, 23 commands, a knowledge base that scores and decays, and an MCP server for semantic search over it — organised as a seven-layer cognitive architecture.

**At a glance:** 52 active hooks, 23 skills, 16 inter-layer bridges, 27 domain nodes — tests green.
...
[![version](https://img.shields.io/badge/version-1.28.1-blue)](CHANGELOG.md)
...
<sub>The author's base after four months, rendered live from <code>~/.claude/global-lessons/</code> by the MCP server's <code>open_graph</code>. 351 items in the base; the graph draws 287 of them as nodes and 64 relations as edges. A fresh install starts with 30 seed items and grows from your own sessions. (UI shown in Russian; the graph itself is language-agnostic.)</sub>
...
**What this does not do:** `blocker: true` does not hard-stop the call. It is a silent marker into the agent's context, deliberately — [the hook says so in its own header](hooks/blocker-tier-check.sh), because noise is its own failure mode. Five separate hooks *do* interrupt: `bash-cost-detector`, `user-correction-guard`, `bulk-copy-guard` and `internal-doc-leak-guard` ask you to confirm; `playwright-cli-guard` refuses outright. Promotion to `blocker: true` is also an explicit decision, never automatic on a counter — each one costs time on every tool call.
...
**The diagnosis was not "typo".** A rule that only a human can enforce is a broken rule. It was filed as a knowledge-action gap — the sixteenth instance of an existing pattern.
...
**Since then the machine catches them, not the human:** 176 findings over five months.
...
Sessions are counted from the L6 telemetry (`intrusiveness-history.jsonl`, 303 unique sessions over the span). The session registry counts 375 for the same period, which would make the fall steeper, not shallower — the more conservative denominator is the one used here.
...
The claim that survives all three: this class of error is now found by a machine 176 times, and it used to be found by a person. The size of the improvement is not established.
...
| 7 | Co-cognition | Did the agent contribute a hypothesis, or only execute? | Emergent — 27 items recorded with `origin: co-cognition`; the disagreement loop has code (73% of 236 events resolved), the layer itself has none |
...
|---|---|---|
| Recall the rule before starting | `knowledge-activator`, 9 anchors, PreToolUse | 12,267 injections; leader `principle-verify-before-acting`, 1,114 |
| Recognise a familiar pit in advance | `blocker-tier-check` + `detection-signals-lib` | 148 pre-action markers across 64 sessions |
| Pull the hand back from a destructive command | `trust-guard`, PreToolUse[Bash] | 24 firings |
| Take a failure apart and write it down | `error-tracker` → `five-whys-gate` → `/learn` | 148 of 212 recorded cases came from a failure |
| Turn separate incidents into a rule | promotion rules, `source_cases` field | 212 cases → 31 patterns, 9 principles; lineage on 40 of 40 |
...
nothing; the pattern base grew from 8 to 31, scoring now always finds something, and the door
...
generalisation has stalled: nine principles, unchanged across ten weekly digests. Learning
from success is the weaker half — 35 success cases against 148 failures. And nothing has ever
been retired: `contradicted_count` is zero everywhere, so the decay machinery has never had
anything to act on.
...
**On its own.** Session start: registry, narrative brief, six startup signals. Every prompt: current time, interlocutor-state classification, deferred alerts. First tool call: the base is scored across nine anchors and the top three items enter the agent's context silently — ranks 4 to 6 are logged as a control group. Before shell calls: destructive-command guard, repeat-failure detector, five-whys gate, changelog and module-doc reminders. On stop: session finalization, uncaptured-lesson prompts, and a timestamp canary that detects when instructions have drifted out of context.

Of the 52 hooks, 45 are wired to Claude Code events, three run on a schedule, and four are called by other scripts or by hand.
...
- **The knowledge-to-action gap is real and quantified.** Of 351 items (measured 2026-08-25), 100% are stored, 25 (7.1%) have ever reached the agent's context, and 4 (1.1%) stand in the way of an action. Of those four, one expresses a rule rather than a list. Storing is easy; acting is not.
- **`contradicted_count` is zero across the entire base.** A counter that only goes up measures nothing. The refutation loop exists in code and has produced no record — the whole contradiction column in the table above is therefore design, not evidence.
...
Hook, skill, bridge and domain counts in this file are generated by `scripts/count-stats.sh`, and a CI guard compares them against reality.
